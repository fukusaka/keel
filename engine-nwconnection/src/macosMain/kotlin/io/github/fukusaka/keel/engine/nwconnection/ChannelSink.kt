...
/**
 * Bridges [NwChannel] to kotlinx-io [RawSink] for codec layer integration.
 *
 ...
 * Each [write] call buffers data; [flush] triggers the actual NWConnection send.
 *
 * Uses [runBlocking] to bridge the suspend [NwChannel.write]/[NwChannel.flush]
 * into the non-suspend [RawSink.write]/[RawSink.flush]. Must not be called from
 * the NWConnection's dispatch queue (caller runs on a coroutine dispatcher).
 *
 * Engine-layer code should use [NwChannel.write]/[NwChannel.flush] directly
 * for lower overhead.
 */
internal class ChannelSink(
    ...
    override fun write(source: Buffer, byteCount: Long) {
        ...
        while (remaining > 0) {
            val chunkSize = remaining.coerceAtMost(NwChannel.CODEC_BUFFER_SIZE)
