...
public class LongObjectMap<V : Any>(initialCapacity: Int = DEFAULT_CAPACITY) {
     ...
     * keys is yielded N times. Stated because a caller cannot tell from the
     * name: the readiness ledgers key on `(fd, interest)` and can hold one
     * value under both keys, so a walk there yields it twice. Whether a
     * consumer wants that or dedupes is the consumer's to decide — what it must
     * not do is assume.
     *
