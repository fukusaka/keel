 ...
 *
 * **Path taxonomy** (mirrors the branches in [PooledAllocator.allocate]):
 * - **hit** ([recordHit]): pool freelist served the request without touching the
 *   chunk-arena. The hot path; cheap, lock-free.
 * - **miss** ([recordMiss]): freelist was empty for this class → `chunkArena.carve`
 *   ran. The contended path; where the planned mutex / locks fire.
 * - **empty** ([recordEmpty]): `allocate(0)` — the empty-buffer marker, neither
 *   pooled nor chunk-backed.
 * - **huge** ([recordHuge]): request exceeds [PooledAllocator.MAX_CACHED_CAPACITY]
 *   — straight `newBuffer(capacity)` per system allocation, no pool involvement.
 *
 ...
 */
class PoolMissProfile(private val nSizes: Int) {
