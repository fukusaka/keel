package io.github.fukusaka.keel.tls.nodejs

import kotlin.js.Promise
import kotlin.test.Test

/**
 * Minimal TLS echo test using Node.js tls module.
 *
 * Validates:
 * - tls.createServer with PEM cert/key
 * - TLS handshake with curl client
 * - HTTP response over TLS
 *
 * Uses dynamic JS interop (no @JsModule) for simplicity.
 */
class NodeTlsEchoTest {

    @Test
    fun node_tls_server_handshake_and_echo_succeeds(): Promise<Unit> {
        return Promise { resolve, reject ->
            try {
                val tls = js("require('tls')")
                val childProcess = js("require('child_process')")

                val options = js("{}")
                options.key = SERVER_KEY
                options.cert = SERVER_CERT

                val server = tls.createServer(options) { socket: dynamic ->
                    socket.on("data") { data: dynamic ->
                        val received = data.toString("utf8") as String
                        println("Server received ${received.length} bytes: ${received.lines().first()}")

                        val body = "Hello, Node TLS!"
                        val response = "HTTP/1.1 200 OK\r\nContent-Length: ${body.length}\r\nConnection: close\r\n\r\n$body"
                        socket.write(response)
                        socket.end()
                    }
                }

                server.listen(0) {
                    // Start curl client after server is listening (port 0 = OS assigns).
                    val port = server.address().port
                    val curl = childProcess.spawn("curl", js("[\"-k\", \"-s\", \"https://localhost:\" + port + \"/hello\"]"))
                    curl.on("close") { _: dynamic ->
                        server.close {
                            resolve(Unit)
                        }
                    }
                    curl.on("error") { err: dynamic ->
                        server.close()
                        reject(Exception("curl failed: $err"))
                    }
                }
            } catch (e: Throwable) {
                reject(e)
            }
        }
    }

    companion object {
        private const val SERVER_CERT = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"""

        private const val SERVER_KEY = """-----BEGIN PRIVATE KEY-----

XDEgg9dRd2akza/XK5Hj
-----END PRIVATE KEY-----"""
    }
}
