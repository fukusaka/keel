...
class NwConnectionQueueDispatcherAssertTest {
    ...
    // Hang-detection budget for the two async cases. Each case dispatches
    // a single block onto a freshly created GCD serial queue and waits on
    // a `CompletableDeferred`. Loopback GCD `dispatch_async` latency is
    // well under 10 ms; 5 s is generous enough to absorb CI runner load
    // and short enough to surface a real hang quickly.
