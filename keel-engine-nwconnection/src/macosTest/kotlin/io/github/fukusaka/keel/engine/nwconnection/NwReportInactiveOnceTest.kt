 ...
 */
class NwReportInactiveOnceTest {
    @Test
    ...
    @Test
    fun `a receive re-armed after a peer FIN completes with the same end and reports nothing`() = runTest {
        withPeerFin(IoEngineConfig(threads = 1)) { f ->
            assertEquals(1, f.reports, "the FIN is reported once")
            // The caller disables and re-enables reads: the re-armed receive
            // completes with the same end of file, which is not news.
            f.onLoop {
                f.transport.readEnabled = false
                f.transport.readEnabled = true
            }
            delay(SETTLE_MS)
            assertEquals(1, f.reports, "a receive re-armed after the FIN must not report it again")
    ...
    private companion object {
        ...
        /** Long enough for a re-armed loopback receive to have completed. */
