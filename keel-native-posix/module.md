...
| `AbstractPosixReadinessEventLoop` | Base class for the epoll and kqueue loops. Requires `@InternalPosixEventLoopApi`. Owns both registration ledgers — the FIFO chain of suspend waiters per `(fd, interest)` and the pipeline-path callback listeners — their shared mutex, and the rule that arming syscalls run on the loop thread. A subclass supplies `inEventLoop()`, `submitArm` and `drainTasks` |
