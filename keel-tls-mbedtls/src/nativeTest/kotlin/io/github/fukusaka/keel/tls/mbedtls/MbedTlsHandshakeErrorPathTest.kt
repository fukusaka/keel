 ...
 * into `null + send_written` — is fixed identically across all three
 * native backends and is regression-covered by the OpenSSL and AWS-LC
 * success-then-close controls (those backends *can* complete an
 * in-memory handshake). It is reachable on the MbedTLS server path in
 * production (handshake completes, the server writes a response via
 * `protect`, then `close()` emits close_notify).
 */
