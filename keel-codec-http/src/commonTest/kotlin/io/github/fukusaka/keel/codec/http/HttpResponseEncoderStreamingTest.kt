...
class HttpResponseEncoderStreamingTest {
    ...
    //
    // Surfaced by `:keel-server-ktor`'s `respondUpgrade` (Step 1).
    // The WebSocket handshake response is `101 Switching
    // Protocols` with no body — neither Content-Length nor
    // Transfer-Encoding applies. Before the BODYLESS state was
    // added the encoder rejected such heads with
    // "HttpResponseHead must declare either Content-Length or
    // Transfer-Encoding: chunked", which silently broke every
    // ktor-keel-* engine's WebSocket upgrade and showed up only
    // when `bench-stream-one.sh ws-echo` reported `checks=0%`.
