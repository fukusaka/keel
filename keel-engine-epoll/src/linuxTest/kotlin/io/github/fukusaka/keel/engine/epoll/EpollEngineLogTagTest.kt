 ...
 * one shared base, the engine-level logger took the base class's name, so epoll
 * and kqueue became indistinguishable in the log and every rule keyed on
 * `EpollEngine` stopped matching — silently, because a tag that matches nothing
...
class EpollEngineLogTagTest {
    ...
    fun `engine asks for its own name and not the base it shares`() = runBlocking {
        withTimeout(ENGINE_CLOSE_TIMEOUT) {
            ...
            val engine = EpollEngine(IoEngineConfig(loggerFactory = factory))
            try {
                assertTrue(
                    "EpollEngine" in factory.tags,
