...
/**
 * Covers [EpollEventLoop.registerIf]'s reason for existing: deciding whether a
...
class EpollRegisterIfSeamTest {
    ...
    @BeforeTest
    fun setUp() {
        eventLoop = EpollEventLoop(NoopLoggerFactory.logger("EpollRegisterIfSeamTest"))
        ...
        withTimeout(TEST_TIMEOUT) {
            ...
            val outcome = runCatching {
                suspendCancellableCoroutine<Unit> { cont ->
                    val reg = eventLoop.registerIf(fd, EpollEventLoop.Interest.READ, cont) { true }
    ...
    fun `registerIf refuses when the caller has already given up`() = runBlocking {
        withTimeout(TEST_TIMEOUT) {
            ...
            suspendCancellableCoroutine<Unit> { cont ->
                val reg = eventLoop.registerIf(fd, EpollEventLoop.Interest.READ, cont) { false }
            ...
            eventLoop.cancelAll(fd, EpollEventLoop.Interest.READ, CancellationException("nothing to cancel"))
        }
    }
