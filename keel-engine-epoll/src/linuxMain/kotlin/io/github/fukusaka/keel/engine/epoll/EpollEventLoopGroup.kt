 ...
 *   a sequence that spans them — which is what reaches the rollback below,
 *   since it needs one loop's construction to fail after others have succeeded.
...
internal class EpollEventLoopGroup(
    ...
    private val loops: Array<EpollEventLoop>
    ...
    init {
        ...
        // built -- each holding an epoll fd, a wakeup eventfd, native scratch
        // and an allocator child that only its own `close()` returns. Nothing
        // else has a reference to them, so nothing ever will.
        val built = ArrayList<EpollEventLoop>(size)
        try {
            repeat(size) {
                built +=
                    EpollEventLoop(
     ...
     */
    private fun closeAll(toRelease: List<EpollEventLoop>, cause: Throwable) {
