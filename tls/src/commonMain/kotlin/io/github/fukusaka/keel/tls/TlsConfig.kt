 ...
 *
 * A single [TlsConfig] can be shared by multiple [TlsSession] instances.
 * Implementations may cache the native config internally for reuse.
 *
 ...
 *
 * **QUIC consideration**: the current interface is designed for TCP TLS.
 * QUIC support (Phase 12+) will add transport parameters and encryption-level
 * callbacks via a separate `TlsHandshaker` super-interface extracted from
 * [TlsSession] at that time. See `research.md` §12–§14 for details.
 */
data class TlsConfig(
     ...
     * via [TlsSession.negotiatedProtocol] after handshake.
     */
