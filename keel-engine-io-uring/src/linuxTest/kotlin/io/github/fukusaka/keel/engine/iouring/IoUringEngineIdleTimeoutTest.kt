 ...
 * io_uring engine ([IoEngineConfig.idleTimeoutMillis]) — the time-axis defence
 ...
 * is small ([IDLE_MS]) so the closing case resolves well inside the envelope. The
 * io_uring loop bounds its `io_uring_submit_and_wait` by the nearest connection
 * deadline, so the timer fires without an external wakeup. A minimal echo /
 * big-chunk handler is used rather than the HTTP codec.
 */
@OptIn(ExperimentalForeignApi::class)
class IoUringEngineIdleTimeoutTest {
    ...
    fun `a silent client is closed after the idle timeout`() = runBlocking {
        withTimeout(15.seconds) {
            val engine = IoUringEngine(IoEngineConfig(threads = 1, idleTimeoutMillis = IDLE_MS))
