...
abstract class AbstractPosixEngine(
    ...
    /**
     * Rejects a Unix address this platform cannot bind or connect to.
     *
     * Linux has an abstract namespace and macOS does not, so what is a valid
     * address on one is not on the other. Nothing by default; the engine whose
     * kernel is narrower says so.
     */
    protected open fun validateUnixAddress(address: UnixSocketAddress) {}

    /**
     * Runs [release], attaching any failure to [cause] rather than letting it
     * replace the reason the rollback is happening.
     *
     * The concrete engines build their loops and need this for the unwind; it
     * lives here so both reach the same one.
     */
    protected inline fun closeQuietly(cause: Throwable, release: () -> Unit) {
        try {
            release()
