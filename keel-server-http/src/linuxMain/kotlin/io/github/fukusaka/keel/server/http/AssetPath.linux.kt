 ...
 *
 * Linux's `struct stat` carries the modification time in the `st_mtim`
 * `timespec` field — distinct from macOS's `st_mtimespec`, hence a
 * Linux-specific actual.
 */
@OptIn(ExperimentalForeignApi::class)
internal actual fun assetLastModified(path: String): Instant? = memScoped {
    val info = alloc<stat>()
    if (stat(path, info.ptr) != 0) return null
    val mtim = info.st_mtim
