...
/**
 * Pins that [TlsConfig.verifyHostname] drives JSSE client hostname
 * verification (`SSLParameters.setEndpointIdentificationAlgorithm("HTTPS")`).
 *
 * The server certificate is self-signed with `CN=localhost` and is trusted
 * via a [TlsTrustSource.Pem] anchor, so chain verification passes regardless
 * of the name. The tests pin the hostname axis on top:
 *
