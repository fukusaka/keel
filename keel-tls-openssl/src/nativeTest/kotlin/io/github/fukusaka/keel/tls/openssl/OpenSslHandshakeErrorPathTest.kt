...
/**
 * Handshake **error-path** coverage for the OpenSSL backend.
 *
 * The existing OpenSSL tests drive only the happy path (`OpenSslEchoTest`
 * / `OpenSslHttpsEchoTest` complete a handshake against a trusted
 * server) or config-creation failures (`OpenSslCodecTest`). Neither
 ...
 * families (JSSE `SSLEngine`, OpenSSL memory-BIO) are pinned to the same
 ...
 * the server regardless of `verifyMode`), OpenSSL honours `verifyMode`,
 ...
 */
class OpenSslHandshakeErrorPathTest {

    private val allocator: BufferAllocator = DefaultAllocator
    private val factory = OpenSslCodecFactory()
    private val pump = OpenSslHandshakePump(allocator)
    ...
    fun `a REQUIRED server aborts when the client presents no certificate`() {
        // Mutual TLS: the server demands a client cert (REQUIRED →
        // SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT) but the client
        // has none configured, so the server aborts the handshake.
        val server = factory.createServerCodec(
    ...
    fun `a PEER server completes when the client presents no certificate`() {
        // Contrast with REQUIRED: PEER requests a client cert but does not
        // demand it, so a cert-less client still completes the handshake.
        val server = factory.createServerCodec(
