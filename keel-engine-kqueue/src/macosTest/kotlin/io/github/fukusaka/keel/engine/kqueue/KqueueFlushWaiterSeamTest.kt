...
internal class KqueueFlushWaiterSeamTest {

    private val logger = NoopLoggerFactory.logger("KqueueFlushWaiterSeam")
    ...
    fun `a FIN attempt that throws does not take the waiter's answer with it`() = runBlocking {
        withTimeout(BUDGET) {
            ...
            val tracker = TrackingAllocator()
            val transport = KqueueIoTransport(fd, eventLoop, tracker, refusingHalfClose)
