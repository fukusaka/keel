 ...
 *
 * Split from [KqueueAcceptFailureSeamTest], which covers the pipelined
...
class KqueueChannelAcceptGuardSeamTest {
    ...
    fun `a Channel-mode accept whose socket cannot be prepared releases the descriptor`() = runBlocking {
        withTimeout(15.seconds) {
            ...
            val scriptedLocal = InetSocketAddress(Host.Ip(IpAddress.parse("0.0.0.0")), 18093)
    ...
    fun `a Channel-mode accept whose worker swept before the attach releases the descriptor`() = runBlocking {
        withTimeout(15.seconds) {
            ...
            val doomed = socket(AF_INET, SOCK_STREAM, 0)
            assertTrue(doomed >= 0, "could not open a socket to be accepted")
            val scriptedLocal = InetSocketAddress(Host.Ip(IpAddress.parse("0.0.0.0")), 18097)
            val fakeSocket = FakeNativeSocket().apply {
                enqueueAccept(sentinelFd, AcceptResult.Accepted(doomed))
            }
    ...
    @Test
    fun `a report that throws does not take the accept's answer with it`() = runBlocking {
        withTimeout(15.seconds) {
            // `Logger` is a public SPI, so a throw out of one is the caller's
            // code running in the middle of a cleanup. Both reports a failed
            // accept makes stand between a descriptor and its release, or
            // between the guard and the `throw` that answers `accept()` -- so
            // an unguarded report would strand the descriptor and hand the
            // caller a logging failure in place of the reason it asked about.
            val warns = ThrowingReportLogger()
            val bossLoop = KqueueEventLoop(warns)
            val sweptGroup = KqueueEventLoopGroup(1, warns, DefaultAllocator)
            val sentinelFd = newSentinelFd()
            val doomed = socket(AF_INET, SOCK_STREAM, 0)
            assertTrue(doomed >= 0, "could not open a socket to be accepted")
            val scriptedLocal = InetSocketAddress(Host.Ip(IpAddress.parse("0.0.0.0")), 18099)
            val fakeSocket = FakeNativeSocket().apply {
                enqueueAccept(sentinelFd, AcceptResult.Accepted(doomed))
            }
            val server = KqueueStreamServer(
            ...
            try {
                ...
                val raised = assertFailsWith<CancellationException> { server.accept() }

                assertTrue(warns.threw, "the report must have refused for this test to mean anything")
                assertTrue(
                    raised.suppressedExceptions.any { it is InjectedFault },
                    "the refusal is carried out on the cause rather than replacing it",
                )
                assertFalse(
                    stillOpen(doomed),
                    "and a report that refused must not cost the descriptor its release",
                )
                server.close()
            } catch (t: Throwable) {
                close(sentinelFd)
    ...
    /**
     * Refuses the accept guard's own report, and records nothing else.
     *
     * Distinct from [ThrowingCloseLogger], which refuses the *teardown's*
     * message so the release throws. This one refuses the message that reports
     * the drop, which is the statement standing between the descriptor and its
     * release.
     */
    private class ThrowingReportLogger : Logger {

        private val refused = AtomicInt(0)

        /** Whether the drop report was seen, and refused. */
        val threw: Boolean get() = refused.value == 1

        override fun isLoggable(level: LogLevel): Boolean = level == LogLevel.WARN

        override fun rawLog(level: LogLevel, throwable: Throwable?, message: Any?) {
            if (level != LogLevel.WARN) return
            if ("dropping the connection" in message.toString()) {
                refused.value = 1
                throw InjectedFault("the logger refused the drop report")
            }
        }
    }
