...
エンジンが EventLoop スレッド上でハンドラを直接呼び出します — suspend なし、コンテキストスイッチなし。ソケットが読み取り可能になると、EventLoop がデータを `IoBuf` に読み込み、`ChannelPipeline` のインバウンドイベントを発火します: データは TLS → デコーダ → ユーザーハンドラ → エンコーダと EventLoop スレッドを離れることなく同期的に流れます。トレードオフ: ハンドラが EventLoop スレッドをブロックしてはなりません。
...
**接続ごとの TLS**（kqueue、epoll、io_uring、NIO、Netty）— `TlsHandler` を `ChannelPipeline` にインストールし、選択した TLS バックエンドで各 `IoBuf` を暗号化・復号します。`keel-engine-netty` では `keel-tls-jsse` を使う方法のほか、`NettySslInstaller`（Netty 内蔵の `SslHandler`）を使う方法もあり、後者は `keel-tls-*` モジュール不要です。
