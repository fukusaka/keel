...
1. **write は所有権を移譲する。** `channel.write(buf)` / `sink.write` / `ctx.propagateWrite` の後、buf は呼び出し側から消える — 触らない、`release()` しない、index も見ない。engine が flush 完了後に release する。write が **throw** したときも同じ — 受け付けなかった channel は、その拒否が呼び出し側に届く前に buf を release しているので、`catch` で release するものは無い。例外は `channel.write` に書くものが無い buf を渡した場合だけで、これは何も受け取らず `0` を返し、buf は呼び出し側のまま。`sink.write` と `ctx.propagateWrite` は空の buf も他と同じように受け取る。
