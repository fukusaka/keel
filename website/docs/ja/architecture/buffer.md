...
1. **write は所有権を移譲する。** `channel.write(buf)` / `sink.write` / `ctx.propagateWrite` の後、buf は呼び出し側から消える — 触らない、`release()` しない、index も見ない。engine が flush 完了後に release する。`channel.write` が **throw** したときも同じ — その時点で buf は release されているか pipeline に渡っているかのどちらかなので、`catch` で release するものは無い。例外は書くものが無い buf を渡した場合で、`channel.write` と、`channel.asSuspendSink()` が返す sink がこれにあたる — 何も受け取らず `0` を返し、buf は呼び出し側のまま。`ctx.propagateWrite` は空の buf も他と同じように受け取る。
