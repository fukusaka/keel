...
Accept した各接続は `Pipeline` — ハンドラの順序付きチェーン — を持ちます。受信イベントは HEAD から TAIL へ、送信操作は呼び出し元ハンドラから HEAD 方向へ流れます。HEAD が `IoTransport` を通じて実際のソケット I/O を担います。

`ctx.propagateWrite()` は呼び出し元ハンドラの位置から **HEAD 方向に向かって** 伝播し、途中の `OutboundHandler` を順に呼び出します。TAIL 側にあるハンドラは通過しません。このことがエンコーダの配置場所を決定します（[Pipeline](#channelpipeline) 参照）。
