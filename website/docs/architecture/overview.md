...
`engine.bind()` is a `suspend fun` that returns a `StreamServer`. `server.accept()` is a `suspend fun` that resumes when the EventLoop detects a new connection. Once accepted, `channel.read(buf)` fills a caller-supplied `IoBuf`, suspending until data arrives — code reads sequentially without blocking a thread. Integrates naturally with Ktor and all kotlinx coroutines primitives. Tradeoff: a coroutine context switch on each `read()` resume.
...
**Coroutine mode**: the EventLoop resumes the suspended coroutine when I/O is ready (epoll, kqueue, NIO, Netty) or complete (io_uring, NWConnection). The coroutine runs directly on the EventLoop thread — keel aligns `channel.ioDispatcher` with the thread that drives the engine's native I/O primitive, so there is no cross-thread handoff between I/O notification and coroutine resumption on any engine. User handlers that need to perform blocking or CPU-intensive work should explicitly offload via `withContext(Dispatchers.IO)` / `withContext(Dispatchers.Default)`.
...
Codec modules do not depend on any `keel-engine-*` module — both `keel-codec-http` and `keel-codec-websocket` depend only on the engine-independent `keel-io` / `keel-core` abstractions (`keel-codec-http` additionally on `keel-compression` for content encoding). Their I/O boundary is the `IoBuf` pipeline handler: decoders receive pooled `IoBuf`s as `TypedInboundHandler<IoBuf>`, and encoders emit `IoBuf`s via `ctx.propagateWrite`; the suspending helpers (`BufferedSuspendSource` / `BufferedSuspendSink` from `keel-io`) form a secondary layer for the coroutine-mode parsers. Neither codec knows which engine is running underneath. If codecs depended on engine internals, each of the 7 engines would need its own codec implementation and unit tests would require a running engine. Instead, a single codec implementation serves all engines and codec tests run in-memory with no engine involved.

The buffer strategy respects the same layering: pooled `IoBuf` is the codec I/O boundary, and variable-length accumulation (frame reassembly, body aggregation) goes through keel-io's chunk-based carriers (`IoBufAccumulator` / `IoBufChunks`) rather than a growing `ByteArray`. kotlinx-io's GC-managed `Buffer` remains only as an internal parsing tool inside codecs, where a GC-owned scratch structure avoids burdening pure parsing logic with reference counting. Engines operate at packet granularity (every recv syscall), so `IoBuf` + `BufferAllocator` gives complete allocation control and keeps GC off the I/O hot path.
