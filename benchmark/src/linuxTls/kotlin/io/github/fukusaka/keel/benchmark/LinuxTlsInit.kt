...
/** Auto-register Linux TLS backends (OpenSSL, AWS-LC, Mbed TLS) at process startup. */
@OptIn(ExperimentalStdlibApi::class)
@EagerInitialization
private val linuxNativeTlsInit: Unit = registerTlsProvider { backend ->
    when (backend) {
        "openssl" -> OpenSslCodecFactory()
        "awslc" -> AwsLcCodecFactory()
        "mbedtls" -> MbedTlsCodecFactory()
        else -> error("Unsupported TLS backend on Linux: $backend (available: openssl, awslc, mbedtls)")
    }
}
