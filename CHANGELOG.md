...
- `engine-nodejs`: split the 591-line `NodeEngineTest` into 4 focused files (`NodeEngineLifecycleTest` / `NodeEngineReadWriteTest` / `NodeEngineConnectTest` / `NodeEngineConcurrencyTest`) plus a shared `NodeEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / concurrency category. 25 tests preserved (#380)
- `engine-nwconnection`: split the 918-line `NwEngineTest` into 5 focused files (`NwEngineLifecycleTest` / `NwEngineReadWriteTest` / `NwEngineConnectTest` / `NwEngineConcurrencyTest` / `NwEngineResourceTest`) plus a shared `NwEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / concurrency / resource category. 34 tests preserved (#379)
- `engine-netty`: split the 955-line `NettyEngineTest` into 5 focused files (`NettyEngineLifecycleTest` / `NettyEngineReadWriteTest` / `NettyEngineConnectTest` / `NettyEngineConcurrencyTest` / `NettyEngineResourceTest`) plus a shared `NettyEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / concurrency / resource category. 36 tests preserved (#378)
- `engine-nio`: split the 1086-line `NioEngineTest` into 5 focused files (`NioEngineLifecycleTest` / `NioEngineReadWriteTest` / `NioEngineConnectTest` / `NioEngineConcurrencyTest` / `NioEngineResourceTest`) plus a shared `NioEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / concurrency / resource category. 40 tests preserved (#377)
- `engine-io-uring`: split the 1165-line `IoUringEngineTest` into 5 focused files (`IoUringEngineLifecycleTest` / `IoUringEngineReadWriteTest` / `IoUringEngineConnectTest` / `IoUringEngineResourceTest` / `IoUringEngineUringSpecificTest`) plus a shared `IoUringEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / resource buckets plus an io_uring-specific bucket (wakeup SQE retry / ASYNC_CANCEL / multishot accept / round-robin EL assignment / close-while-armed). 41 tests preserved (#376)
- `engine-epoll`: drop 57 unused imports inherited from PR #373 monolith split (8 + 14 + 12 + 10 + 13 across `EpollEngine{Lifecycle,ReadWrite,Connect,Concurrency,Resource}Test`); no test logic changes (#375)
- `engine-kqueue`: split the 1394-line `KqueueEngineTest` into 6 focused files (`KqueueEngineLifecycleTest` / `KqueueEngineReadWriteTest` / `KqueueEngineConnectTest` / `KqueueEngineConcurrencyTest` / `KqueueEngineResourceTest` / `KqueueEnginePipelineTest`) plus a shared `KqueueEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / concurrency / resource / kqueue-pipeline category. No test logic changes; all 52 tests preserved (#374)
- `engine-epoll`: split the 1302-line `EpollEngineTest` into 5 focused files (`EpollEngineLifecycleTest` / `EpollEngineReadWriteTest` / `EpollEngineConnectTest` / `EpollEngineConcurrencyTest` / `EpollEngineResourceTest`) plus a shared `EpollEngineTestSupport` for helpers + constants, organised by lifecycle / read-write / connect / concurrency / resource category. No test logic changes; all 48 tests preserved (#373)
