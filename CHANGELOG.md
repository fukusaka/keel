...
- `engine-io-uring`, `engine-nodejs`, `engine-nwconnection`: write path consumes the multi-segment `IoBuf` iteration API on the final engine triple, completing 7-of-7 engine coverage. io_uring's WRITEV SQE / direct `keel_writev` path expands a multi-seg chain into iovec entries; the zero-copy modes (`SEND_ZC` / `SENDMSG_ZC` and the EAGAIN `submitAsyncSendChain` recovery) downgrade to the writev path when any pending write is multi-segment, and partial-writev recovery resubmits the unwritten suffix as another writev SQE via the new `submitAsyncWritevSuffix`. nodejs dispatches one `socket.write()` per chained segment (Node's stream layer batches the sequential writes into the underlying TCP send). nwconnection routes multi-seg or multi-buf flushes through `keel_nw_writev_async` (single-seg single-buf still uses `keel_nw_write_async`)
- `engine-epoll`, `engine-netty`: write path consumes the multi-segment `IoBuf` iteration API on the second engine pair — epoll mirrors the kqueue change (one writev iovec entry per chained segment via `IoBuf.appendSegmentsForRange`, single-seg single-buf fast path preserved); netty expands a multi-seg buffer into one Netty `ByteBuf` (`Unpooled.wrappedBuffer` over each segment's direct `ByteBuffer` window) per chained segment, written in order through `nettyChannel.write` / `writeAndFlush` so Netty's outbound pipeline batches them into a single TCP send. `NettyByteBufIoBuf`'s zero-wrap `retainedSlice` path is unchanged — `NettyByteBufIoBuf` is always single-segment
- `engine-kqueue`, `engine-nio`: write path consumes the multi-segment `IoBuf` iteration API — the `writev` (kqueue) / `SocketChannel.write(ByteBuffer[])` (nio) gather flush builds its iovec / ByteBuffer array via `IoBuf.appendSegmentsForRange`, so a multi-seg buffer expands into one entry per chained segment. Single-seg single-buffer flush keeps the existing fast path (one `write()` syscall, no iovec assembly). Loopback bench (4 KiB payload, JVM nio) reports `single-seg 36 μs → multi-seg 4 × 1 KiB 47 μs (+31 %) / 8 × 512 B 58 μs (+62 %)`, all comparing against the pre-PR single-write baseline (single-seg path unchanged)
- `io`: `BufferAllocator.allocate(capacity, maxCapacity)` and `BufferAllocator.allocateSegment(capacity)` — the public production path that codec / engine code uses to build a multi-segment-capable buffer plus the standalone `Segment`s to chain into it. `DefaultAllocator` provides platform-typed implementations on JVM / Native / JS; pool allocators inherit the default `UnsupportedOperationException` until the codec workaround-retire stack PR wires them up
- `io`: `IoBuf.appendSegmentsForRange(offset, length, into)` and `IoBuf.segmentCount` — the lowest-level multi-seg surface engines consume to assemble scatter-gather descriptors over an explicit `(offset, length)` snapshot of an `IoBuf`. `SegmentRangeList.clear` is now public so cross-module callers (engines) can reset the scratch list at the start of each flush. Internal `SegmentChain.appendReadableSegments(...)` complements `fillReadableSegments` for the append-without-reset accumulation pattern
- `io`: `IoBuf` surface now multi-segment-capable — new `maxCapacity` property, `appendSegment(seg)` to extend the chain (throws `KeelBufferOverflowException` at the cap), and two iteration APIs (`forEachReadableSegment` / `fillReadableSegments`). The three platform implementations (`DirectIoBuf` / `NativeIoBuf` / `TypedArrayIoBuf`) override the defaults to delegate to an internal `SegmentChain`; each preserves the single-segment hot path (primary segment access at index < primary capacity, slow-path locate-and-walk only when crossing) and keeps the single-seg representation at one extra allocation (the chain object; the extras list is lazy). Slicing across segment boundaries is deferred — `sliceWindow` rejects ranges that span the primary→extras boundary. Custom (non-Segment-backed) implementations such as `RingBufferIoBuf` keep the interface defaults and remain effectively single-segment
- `io`: foundation primitives for multi-segment `IoBuf` — public `SegmentBacking` marker with platform extensions (`asByteBuffer` on JVM, `asNativePointer` on Native, `asInt8Array` on JS), an internal `SegmentChain` that holds an ordered list of `Segment`s with explicit `appendSegment` + `maxCapacity` cap, two iteration APIs (`SegmentRangeAction` callback + `SegmentRangeList` boxing-free list) selected by the multi-seg PoC (PR #602, closed), and `KeelBufferOverflowException` thrown when a chain append would exceed the cap. `IoBuf` itself is unchanged in this PR; the multi-segment surface is wired up in the follow-up stack PRs
