...
- keel-engine-nio: `NioServer.close()` is now safe to call from any thread. Previously, when called from a non-EL thread (e.g. a JVM shutdown hook) while an `accept()` coroutine was suspended inside a `DispatchedContinuation`, `resumeWithException` ran on the caller thread outside the continuation's dispatcher. Under wrk-scale shutdown load this leaked `ClassCastException: CompletedContinuation cannot be cast to DispatchedContinuation` from kotlinx.coroutines' `DispatchedContinuation.releaseInterceptedContinuation`. The close teardown (cancel pending continuation, cancel selection key, close server channel) is now re-dispatched onto the boss EventLoop when the caller is not already on it; the resume then runs on the expected dispatcher. Non-EL callers block up to 2 s for the re-dispatched task so `isActive == false` is visible on return.
