...
- `engine-kqueue` / `engine-epoll`: replace the `MutableMap<Long, *>` fd registration tables (`registrations` / `callbackRegistrations`) with the new primitive-keyed `LongObjectMap<V>` (`keel-io`), eliminating per-call `Long` key boxing on the EventLoop dispatch hot path. Micro-bench (PR #358) measured ~3.4x speedup on the put/remove primitive (~33 ns/op → ~9.6 ns/op). `/hello` throughput is syscall-dominated so end-to-end bench shows ±1 % noise; the win is workload-shaped (high-churn / short-lived connection scenarios benefit most) (#360)
...
- `io`: `LongObjectMap<V : Any>` primitive-keyed open-addressing hash map utility — minimal `get` / `put` / `set` / `remove` / `containsKey` / `clear` / `size` API, no `kotlin.collections.Map<Long, V>` interface (avoids the boxing-at-interface-boundary problem of `Map<K, V>.get`). Designed for hot-path data structures where `HashMap<Long, V>` `Long`-key boxing dominates per-call cost; first consumer is the engine fd registration tables in #360 (#360)
