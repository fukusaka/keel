...
- keel-tls: rename the `TlsHandler` companion constant from the generic `OUTPUT_BUF_SIZE` to `TLS_RECORD_BUF_SIZE` so that the three call sites in `processInbound`, `processOutbound`, and `flushHandshakeResponse` make their TLS-record semantic explicit at the allocation point. Value unchanged at `17 * 1024 = 17408`, which covers every reachable wire record produced by the IANA-registered TLS 1.2 / 1.3 cipher suites that keel's TLS backends (JSSE, OpenSSL, MbedTLS, AWS-LC) negotiate (max reachable is 16709 wire bytes for TLS 1.2 CBC + HMAC-SHA384 with maximum random padding; the TLS 1.3 AEAD protocol ceiling per RFC 8446 §5.2 is 16645). Replace the previous 2-line inline comment with a full KDoc block recording the per-variant wire record size table, the RFC 5246 §6.2.3 / RFC 8446 §5.2 / RFC 8449 §1 / RFC 8446 §5.4 citations, the overflow-to-`BUFFER_ERROR` mapping that matches the RFC-mandated receiver response (terminate on `record_overflow`), and the deferred pool-miss decision. The previous inline comment conflated TLS 1.3 AEAD padding (256 bytes) with TLS 1.2 CBC expansion (~1024 bytes) and quoted a "TLS record max ~16645" figure that matched neither reachable maximum of either spec.
