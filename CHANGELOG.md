...
- `benchmark`: `bench-stream-all.sh` places `js:pipeline-http-nodejs` at the head of the engine list so its 50-VU `ws-large` bench runs on a fresh macOS ephemeral port pool. Chain order had it last, after ~14 preceding engines consumed thousands of ephemeral ports each held in TIME_WAIT (2×MSL = 30 s on macOS); the 16,384-port pool saturated and Node.js's single-threaded WS handshake surfaced the shortage as `status 101 0%`. Same macOS class as nodejs/node#32337 (#TODO)
