...
- `benchmark`: `bench-stream-one.sh` skips the `ws-large` scenario for Phase 2 Native reference servers (zig-bench / swift / vertx) that expose a per-frame WebSocket surface and cannot reassemble a single 1 MB message. Zig std.http's `readSmallMessage` returns `error.MessageOversize` and sends a 1003 close immediately; k6 v1.7 `k6/ws` then blocks the JS event loop inside `socket.sendBinary(1 MB)` for 5+ minutes, ignoring `timeout(1)` and SIGTERM. `ws-large.js` also gains a `socket.on('close')` flag + `socket.setTimeout` watchdog as defence-in-depth for graceful-close cases. Override with `BENCH_WS_LARGE_SKIP_BROKEN_REFS=false` (#503)
