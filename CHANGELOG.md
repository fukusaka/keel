...
- `benchmark`: HTTP response compression bench — `compression.js` k6 scenario sends `Accept-Encoding` to `/large` and validates Content-Encoding + decompressed body, opt-in via `BENCH_COMPRESSION_ENABLE=true`. Server side wired through Ktor adapters (gzip + deflate, JVM only via expect/actual; Native gets a no-op), Vertx, Netty raw, and Spring; pipeline-http engines surface the gap as a missing Content-Encoding check failure (#407)
