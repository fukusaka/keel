...
- `StreamEngine.bind(address)` and `StreamEngine.bindPipeline(address)` no longer leak the server-side file descriptor (or JVM `ServerSocketChannel` / Netty `Channel`) when any step between fd acquisition and the returned `ServerChannel` / `PipelinedServer` throws. Affected engines: `EpollEngine` / `KqueueEngine` / `IoUringEngine` (POSIX fd) / `NioEngine` (JVM `ServerSocketChannel`) / `NettyEngine` (Netty `io.netty.channel.Channel`). Previously the success path created the fd via `PosixSocketUtils.createServerSocket` / `createUnixServerSocket` / `createReusePortServerSocket` (or the JVM / Netty equivalent) and only later registered it with the boss EventLoop, constructed the `ServerChannel` / `PipelinedServer`, and called `start()` on the pipeline-mode variant; any exception after fd creation (e.g. `epoll_ctl` / `kevent` failure, server ctor failure, `serverChannel.start()` submit failure — the last of which became visible in PR #307's io_uring submission barrier) left the underlying fd / channel orphaned, silently consuming an OS handle per failed call. Each `bindInet` / `bindUnix` / `bindPipelineInet` / `bindPipelineUnix` now wraps the post-acquisition work in `try { ... } catch (t: Throwable) { close-or-closeFdSafely(...); throw t }` so the resource is released along the error path, preserving the original exception. `IoUringEngine.bindPipelineInet` specifically tracks `createdCount` across the `SO_REUSEPORT` fd-fanout loop so that if the k-th `createReusePortServerSocket` throws, only the k-1 fds actually acquired are closed — the remaining uninitialised zero slots would otherwise be interpreted as stdin and produce a spurious `close(0)` warning. Two private helpers are introduced in `NioEngine` / `NettyEngine` (`closeQuietly(channel, context)`) to swallow secondary exceptions from the close call itself (they are logged at warn level) so the original failure that triggered the cleanup is preserved for the caller.
