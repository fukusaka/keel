...
- `codec-http`: `HttpHeaders` internal storage migrated from `LinkedHashMap × 2` to a list-of-entries (`ArrayList<HeaderEntry>` preserving insertion order) with an `IntArray`-indexed hash bucket (`BUCKET_COUNT=32`) over a per-entry case-insensitive name hash, for HTTP/2 HPACK / HTTP/3 QPACK forward compatibility. Public `String` API unchanged. Per-request `HeaderEntry` is 24 B on JVM (compressed oops, same as a 2-ref class — `Int hashLower` fits in the alignment padding). End-to-end `/hello` allocation -21.7 % vs `LinkedHashMap × 2` baseline (4369 → 3423 B/req, JFR sampled); CDN-mediated production workload (N=23 + 5 lookups) per-cycle pool allocation 552 B (vs 5112 B for LinkedHashMap × 2, -89 %); lookup latency 11 ns flat for `N=1-50` (vs 20-80 ns linear scan over a list). Pool reclaims the `ArrayList` backing array + bucket head `IntArray` + per-entry chain `IntArray` (#5XX)
