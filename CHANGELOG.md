...
- `keel-server-ktor`: `KeelApplicationResponse.respondUpgrade` is now implemented — every `ktor-keel-*` engine (nio / netty / kqueue / nwconnection / epoll / io-uring) accepts WebSocket / protocol upgrade handshakes via Ktor's `webSocket(...)` route block. Internally hijacks the connection by removing the HTTP codec stack and bridging raw transport bytes to a pair of Ktor `ByteChannel`s for the upgrade callback (#409)
...
- `keel-codec-http`: `HttpResponseEncoder` now accepts bodyless 1xx (Informational) / 204 (No Content) / 304 (Not Modified) heads via a new `BODYLESS` streaming mode. Previously rejected any head without `Content-Length` or `Transfer-Encoding: chunked`, which silently broke the WebSocket 101 Switching Protocols handshake on every `ktor-keel-*` engine. RFC 9112 §6 (#409)
