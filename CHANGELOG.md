...
- `native-posix`: `NativeSocket` interface + `PosixNativeSocket` impl with sealed `ReadResult` / `AcceptResult` / `ConnectResult` / `ShutdownResult` (and reused `WriteResult`); EINTR-retrying C wrappers (`keel_read` / `keel_write` / `keel_accept` / `keel_connect` / `keel_send` / `keel_shutdown`; `keel_writev` gains retry) ([#323])
