...
- `server-http`: launch each request handler on a per-connection child scope and drop the no-op `withContext` hop from response writes — cuts shared-`SupervisorJob` child-list contention and a redundant dispatcher round-trip, recovering ~6% throughput at saturation. In-flight handlers are now cancelled when the peer disconnects (#535)
