...
- Dokka: expand `benchmark/module.md` with standalone binary build commands, Phase 2 Native servers (Rust/Go/Swift/Zig), benchmark script reference, and missing JVM pipeline engines (`pipeline-http-nio`, `pipeline-http-netty`); fix inaccuracies: multi-binary description, signal handler Native-only scope, `--connection-close` arg, `BENCH_COOLDOWN`/`BENCH_PORT` env vars
