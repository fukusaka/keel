...
- `keel-server-ktor`, `keel-server-ktor-cio`: bound the WebSocket `upgradeJob` join with a 5-second timeout so the close handshake cannot deadlock when Ktor's `WebSocketSession` is waiting for `inputChannel` EOF that is itself blocked behind `upgradeJob` completion (observed on `ktor-keel-nio` / `ktor-cio-keel-nio` macOS, K23); after the timeout the upgrade job is cancelled and the connection torn down (#TBD)
