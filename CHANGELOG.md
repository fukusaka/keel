...
- `keel-server-ktor-cio`: `KtorCioConnectionHandler` now awaits transport flush completion before closing the connection; chunked SSE streams on `ktor-cio-keel-epoll` / `ktor-cio-keel-io-uring` no longer drop the `0\r\n\r\n` terminator under high concurrency, fixing the k6 gracefulStop stall seen at 50 VUs (#440)
