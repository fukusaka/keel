...
- `benchmark`: detect server crashes in `bench-one.sh` via post-`wait` exit-status signal interpretation. Pre-fix used a pre-`kill_server` `kill -0 $PID` check that missed the timing window where the server SIGSEGVs / SIGABRTs after wrk has already collected a partial `Requests/sec` summary line — the cell silently logged `OK` with a misleading low rps. Post-fix captures `wait $PID`'s real exit code, decodes `128 + signum`, classifies SIGSEGV (139) / SIGABRT (134) / SIGBUS (138 / 135) / SIGFPE (136) / SIGILL (132) as server-originated crashes (SIGTERM / SIGKILL stay attributed to our own `kill_server` / `kill_port`), and overrides the run status to `CRASH` even when wrk reported a numeric rps so the runs array surfaces the failure (`[100K CRASH 99K]`) instead of hiding it inside an apparently-numeric value (#625)
