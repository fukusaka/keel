...
- `io`: `IntArrayPool` (per-EventLoop reuse of fixed-size `IntArray`s) and `IoBufByteCharSequence` (zero-copy `CharSequence` view over an `IoBuf` byte range) — enabling primitives for the HTTP/1.1 headers slot-table refactor (#5XX)
