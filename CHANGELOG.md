...
- `core`: close the socket fd on peer-FIN for Pipeline-mode channels. The peer-close auto-close was deferred to `ensureBridge`, which a Pipeline-mode channel never calls, leaking one fd per connection in CLOSE_WAIT — connection-churn workloads exhausted `RLIMIT_NOFILE` within seconds. The close is now deferred only for an empty (Coroutine-mode, pre-bridge) pipeline (#540)
