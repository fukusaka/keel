...
- `benchmark`: k6 scenarios `benchmark/k6/upload.js`, `sse.js`, and `ws-echo.js` driving the new endpoints, plus `bench-stream-one.sh` helper that mirrors `bench-one.sh`'s contract (start server, run client, parse output, emit `<name>|<rps>|<p50>|<p99>` row) but invokes k6 instead of wrk. The script picks the right k6 metric per scenario (`http_reqs` / `http_req_duration` for HTTP, `ws_msgs_received` / `ws_session_duration` for WebSocket). README under `benchmark/k6/` documents install + run (#394)
