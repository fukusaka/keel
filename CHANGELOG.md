...
- `core`: Coroutine-mode channels are no longer auto-closed on peer-FIN — `read()` reports EOF as `-1` and the caller closes the `Channel`; auto-close is kept for Pipeline-mode channels, where keel owns the connection lifecycle (#541)
