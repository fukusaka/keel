...
- `native-posix`: `NativeSocketOps` cold-path seam — `socket` / `bind` / `listen` / `setsockopt` / `fcntl` / `getsockname` / `getpeername` / `getsockopt(SO_ERROR)` + composite `acceptClient` / `create*ServerSocket` abstracted behind an interface, with `PosixNativeSocketOps` as the production singleton impl (renamed from `PosixSocketUtils`) + `FakeNativeSocketOps` for unit tests. Parallels the `NativeSocket` hot-path seam but kept separate to keep the hot path 8-method narrow ([#334])
