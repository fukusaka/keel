...
- `codec-http`: `StaticHeaderTable` process-wide intern for well-known `(name, value)` pairs — 61 HPACK static entries (RFC 7541), 99 QPACK static entries (RFC 9204), 11 HTTP/1.1 hop-by-hop / framing pairs (RFC 9110 / 9111 / 9112). `HttpHeaders.add` reuses the shared `HeaderEntry` on H1 Tier 2 hit (24 B saved per hit). `hpackStaticEntry(n)` / `qpackStaticEntry(n)` accessors prepare H2 / H3 indexed-entry decode for Phase 13 / 14. Tier 3 production-frequent H1 pairs deferred to a follow-up PR derived from HTTP Archive BigQuery via the QPACK methodology (#592)
