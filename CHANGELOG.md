...
- `keel-codec-http`: `HttpResponseEncoder` chunked encoder no longer crashes with `ArrayIndexOutOfBoundsException` after ~50 frames in a single response. The 256-byte per-encoder scratch buffer for hex framing was being written without a pre-emit bounds check; long SSE / chunked streams (e.g. `/sse-stream?count=100`) overran scratch on the 52nd frame, killing the connection before the `0\r\n\r\n` terminator and breaking HTTP/1.1 keep-alive (99.97 % of follow-up requests failed). Slow path now allocates a fresh exact-sized `IoBuf` for the chunk header / suffix when scratch is full and leaves earlier scratch-backed views intact (#422)
