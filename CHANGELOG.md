...
- `benchmark`: SSE handlers across `NettyRawEngine`, `zig-bench`, `rust-bench`, and `SpringEngine` now produce a wire format compatible with k6's `body size matches` check and (where reachable) flush per frame — `netty-raw` switched per-frame `ctx.write` to `writeAndFlush`, `zig-bench` shrunk its `respondStreaming` buffer to 256 B and added `flush()` per frame, `rust-bench`'s axum `Sse` stream now `yield_now`s between items so hyper can flush each chunk, `spring` writes raw `DataBuffer`s via `BodyInserters.fromDataBuffers` instead of `Flux<String>` (whose SSE adapter mangled our pre-formatted `data: …\n\n` into `data:data: …\ndata:\ndata:\n\n` and broke the body-size check); the SSE rows for these reference servers are now comparable to the keel engines and pass the k6 checks at 100 % (#442)
