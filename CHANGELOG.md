...
- `io`: `IntArrayPool` (per-EventLoop reuse of fixed-size `IntArray`s) and `IoBufAsciiText` (zero-copy `CharSequence` view over an `IoBuf` byte range, ISO-8859-1 semantics) — enabling primitives for the HTTP/1.1 headers slot-table refactor (#5XX)
