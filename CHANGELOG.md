...
- `core`: `DispatchQueueLocal<T>` (appleMain) — Apple-platform per-`dispatch_queue_t` storage, the GCD analogue of `ThreadLocal`. `install(queue, value)` binds a value (GCD owns its `StableRef` lifetime, disposing at queue release), `current()` returns the scoped value or a caller-supplied `fallback()`, `isScopedHere()` reports queue-scope membership. Distinct instances use distinct keys so they coexist on one queue without collision. Used by the K56b fix to give every NWConnection per-connection queue a private `HttpHeadersPool` stack; reusable by any future Apple-target consumer needing per-connection-queue isolation. Apple-only by design — JVM / K/N `@ThreadLocal` already preserve per-pthread isolation for the pthread-pinned engines. (#627)
