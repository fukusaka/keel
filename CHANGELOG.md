...
- **BREAKING** (`engine-kqueue` / `engine-epoll` internal API): `KqueueEventLoop.register()` / `EpollEventLoop.register()` return `Registration` (was `Unit`); `unregister(fd, interest)` is replaced by `unregister(reg: Registration)`. Multiple coroutines registering on the same `(fd, interest)` key now form a FIFO chain instead of overwriting each other in the registrations map — previously every concurrent waiter except the last was silently leaked. The chain is intrusive (`Registration.next` / `tail`) so the common single-waiter path stays alloc-free; new helper `cancelAll(fd, interest, cause)` lets `StreamServer.close()` resume every queued accept caller in one shot. `KqueueStreamServer` / `EpollStreamServer` drop their single-slot `pendingAcceptCont` and atomically check `_active` + register inside the StreamServer mutex so a racing `close()` cannot strand a fresh registration. `EpollEventLoop.dispatchReady` keeps the epoll filter armed when chain siblings remain (cascade-fire) and only `epoll_ctl(MOD)`-disarms when the chain becomes empty. The `echo with multi-thread EventLoop` test on both engines moves blocking POSIX scaffolding (`connectRawClient` / `rawWrite` / `rawRead`) onto `Dispatchers.Default` so single-thread `runBlocking` cannot deadlock when accept FIFO order shifts under load (#367)
