...
- keel-ktor-engine: `KeelApplicationEngine.stop()` no longer waits for the full `timeoutMillis` when idle keep-alive connections are open at shutdown. Root cause of the previous behaviour: `initServerJob` closed `ioEngine` as the final step of the server job body, before the per-connection `handleConnection` children had completed. With idle keep-alive connections present, those children remained suspended on `bridge.receiveCatching()` on their per-channel EventLoop dispatcher. `ioEngine.close()` then shut down that dispatcher; the outer `serverJob.cancel()` during stop's grace expiry could not deliver cancellation to those continuations (the dispatcher was dead), and `serverJob.join()` timed out. Fix: track accepted channels in a `MutableSet` guarded by `Mutex` (`activeChannels`), adding on accept and removing in a `finally` around `handleConnection`. Before `ioEngine.close()`, close every still-active channel so the pipeline read loops observe EOF and the children unwind naturally. Regression test `EngineStopShutdownTest` added: the 20-idle-keep-alive scenario now completes in ~10 ms instead of waiting for the full 1 s timeout, and the benchmark-oriented `Runtime.halt(0)` shutdown-hook fallback introduced in PR #144 is no longer needed to mask this specific case. A separate structural limitation remains for in-flight suspending application handlers (e.g. a route that `delay(Long.MAX_VALUE)`s): when `ioEngine.close()` shuts down the per-connection dispatcher, `cont.cancel()` cannot reach the suspended coroutine, so `stop()` relies on `timeoutMillis` to return in that case. Pinned by `z stop with in-flight suspending handler returns within timeout bound`. A future PR could close `ioEngine` only after every child coroutine has completed to address this residual path.
