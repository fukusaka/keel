...
- `StreamEngine.bind(address)` and `StreamEngine.bindPipeline(address)` no longer leak the server-side resource (POSIX fd for the Native engines, JVM `ServerSocketChannel` for NIO, Netty `io.netty.channel.Channel` for Netty, Network.framework `nw_listener_t` for NWConnection) when any step between resource acquisition and the returned `ServerChannel` / `PipelinedServer` throws. Affected engines: `EpollEngine` / `KqueueEngine` / `IoUringEngine` / `NioEngine` / `NettyEngine` / `NwEngine`. Previously the success path acquired the resource and only later registered it with the boss EventLoop (native engines), constructed the `ServerChannel` / `PipelinedServer`, or waited for the NWListener to reach the `ready` state (NWConnection); any exception after acquisition (e.g. `epoll_ctl` / `kevent` failure, server ctor failure, `serverChannel.start()` submit failure — the last of which became visible in PR #307's io_uring submission barrier — or an NWListener startup timeout / `state == failed` outcome) left the underlying resource orphaned, silently consuming an OS handle per failed call. Each `bindInet` / `bindUnix` / `bindPipelineInet` / `bindPipelineUnix` now wraps the post-acquisition work in `try { ... } catch (t: Throwable) { close-or-cancel(...); throw t }` so the resource is released along the error path, preserving the original exception. `IoUringEngine.bindPipelineInet` specifically tracks `createdCount` across the `SO_REUSEPORT` fd-fanout loop so that if the k-th `createReusePortServerSocket` throws, only the k-1 fds actually acquired are closed — the remaining uninitialised zero slots would otherwise be interpreted as stdin and produce a spurious `close(0)` warning. Three private helpers are introduced: `closeQuietly(channel, context)` in `NioEngine` / `NettyEngine` and `cancelListenerQuietly(lsnr, context)` in `NwEngine`; all three swallow secondary exceptions from the close / cancel call itself (logged at warn level) so the original failure that triggered the cleanup is preserved for the caller. `NwEngine.bindInet` / `bindUnix` additionally null out the engine-level `listener` field when the cancellation is for the same listener it references, so a later `engine.close()` does not attempt a double-cancel on an already-cancelled listener.

### Changed

- **BREAKING (keel-core)**: `Channel.appDispatcher` / `IoTransport.appDispatcher` / `AbstractIoTransport.appDispatcher` / `AbstractPipelinedChannel.appDispatcher` are removed. The property had exactly one hot-path consumer (`KeelApplicationEngine.handleConnection` wrapping `pipeline.execute(call)` in `withContext(channel.appDispatcher)`) and was a Ktor-engine concern leaking into the transport layer. The application-layer dispatch decision now lives in `KeelApplicationEngine.Configuration.applicationDispatcher: CoroutineDispatcher? = null`; when null (the default) the Ktor pipeline runs on `channel.ioDispatcher`, matching every engine's previous `appDispatcher = ioDispatcher` behaviour after PR #311. Callers with a custom `IoTransport` implementation should drop their `appDispatcher` override — it is no longer part of the interface. Callers relying on the old per-engine NIO default (`Dispatchers.Default` for the Ktor pipeline) should set `applicationDispatcher = Dispatchers.Default` explicitly when building the Ktor engine. No source behaviour change for apps using defaults: every engine already collapsed `appDispatcher` to `ioDispatcher` after PR #311, so the default state is preserved. The accompanying `withContext(Dispatchers.IO)` guidance for blocking I/O is unchanged.
- `NioIoTransport` no longer overrides `appDispatcher` to `Dispatchers.Default`; `appDispatcher` is now inherited (= `ioDispatcher` = the `NioEventLoop` Selector thread) to match every other engine (epoll / kqueue / io_uring / Netty / NWConnection / Node.js). The Ktor application pipeline therefore runs on the same thread that drives `Selector.select` / `SocketChannel.read` / `SocketChannel.write`, and the `withContext(appDispatcher) { pipeline.execute(call) }` wrapper in `KeelApplicationEngine.handleConnection` collapses to a no-op (same dispatcher, elided by the coroutine runtime) — zero per-request cross-thread hops. The override was motivated by a 2026-03-27 luna.local measurement (design.md §17) in which EL dispatch regressed ktor-keel-nio by -37% (130K → 82K); re-measured on 2026-04-20, EL dispatch now improves ktor-keel-nio by +9.5% (513,395 → 562,269 req/s, 4t/100c/10s /hello) because the Phase 10 PipelinedChannel / HttpWriter rewrite and `NioEventLoop.dispatch`'s `inEventLoop` wakeup-skip optimisation together remove the overhead that motivated the split. pipeline-http-nio is unchanged (Ktor layer is bypassed). User handlers on the NIO engine are now held to the same non-blocking contract as the other engines: wrap blocking I/O in `withContext(Dispatchers.IO)` by the caller. This is a behavioural change for NIO users that relied on Dispatchers.Default work-stealing to absorb blocking handler code; the mitigation (`withContext(Dispatchers.IO)`) is source-level and matches the idiom required on every other keel engine, including Netty after PR #310.

### Fixed

- `NettyIoTransport.ioDispatcher` now points at the channel's Netty `EventLoop` via the new `NettyEventLoopDispatcher` (a thin `CoroutineDispatcher` wrapper around `EventLoop.execute`, with `isDispatchNeeded` elision via `EventLoop.inEventLoop`) instead of `Dispatchers.Default`. Same structural fix as PR #309 for NWConnection: previously coroutine-side `withContext(ioDispatcher) { bridge.read(buf) }` (from `PipelinedChannel.read`) ran on a `Dispatchers.Default` worker thread while Netty inbound callbacks fired on the channel's `EventLoop`, so `SuspendBridgeHandler.readCont` / `readQueue` / `eof` were mutated from two different threads with no memory-visibility guarantees. The JVM memory model plus Netty's internal barriers keep the race latent in practice, but the structure is identical to the one that deterministically failed on Kotlin/Native in `NwEngineTest`. Aligning `ioDispatcher` with the EventLoop the inbound callbacks already use satisfies `SuspendBridgeHandler`'s documented single-thread invariant the same way the EventLoop-based Native engines (epoll / kqueue / io_uring / nio) already do. `appDispatcher` is left inherited (= EventLoop) so the Ktor application pipeline runs on the same thread as I/O with zero per-request cross-thread hops; user handlers are expected to be non-blocking, matching the Native engines' model. Loopback `/hello` wrk 4t/100c/10s on macOS M1 (5 runs median with shuffle): `ktor-keel-netty` 109,996 → 128,951 req/s (+17%); `pipeline-http-netty` 133,418 → 132,455 req/s (within run-to-run noise, as the pipeline benchmark does not traverse the Ktor layer).
- `NwIoTransport.ioDispatcher` now points at the per-connection NWConnection dispatch queue (`connQueue`) instead of `Dispatchers.Default`, via the new `NwConnectionQueueDispatcher` (a tiny `CoroutineDispatcher` wrapper around `dispatch_async`). This structurally eliminates the cycle-13 stall in `NwEngineTest.GC heap size does not grow after repeated echo cycles` that PR #308 worked around by cutting the echo-cycle loop from 50 to 10: the bug was that `withContext(ioDispatcher) { bridge.read(buf) }` (from `PipelinedChannel.read`) ran on a `Dispatchers.Default` worker while NWConnection receive callbacks fired on the dispatch queue, so `SuspendBridgeHandler.readCont` / `readQueue` / `eof` were mutated from two different threads with no memory-visibility guarantees — a Kotlin/Native concurrent GC cache invalidation around cycle 13 deterministically exposed a stale `readCont = null` view on GitHub Actions `macos-latest`. With `ioDispatcher` pointing at the same dispatch queue NWConnection uses for every callback, coroutine resumptions and native callbacks are serialised on one queue, matching the single-thread invariant every other engine already honours. The `NwEngineTest.GC heap size does not grow` echo-cycle loop is restored to 50 iterations accordingly.
- `NwEngineTest.GC heap size does not grow after repeated echo cycles` no longer fails spuriously on GitHub Actions `macos-latest` runners. The failure was initially misdiagnosed as a 512KB heap-tolerance boundary issue and then as a 3-second `withTimeout` bound being too tight for CI-VM scheduler variance; both were wrong. JUnit XML artifacts with per-cycle timing probes showed the test completes cycles 0-12 in 0 ms each but then deterministically stalls on cycle 13's `ch.read(buf)` for more than 10 seconds. Root cause: `NwIoTransport.ioDispatcher = Dispatchers.Default` while the NWConnection receive callback fires on the dispatch_queue thread — `bridge.read(buf)` (`withContext(Default) { … }`) runs on a Default-pool worker while `bridge.onRead` runs on the dispatch_queue thread, and `SuspendBridgeHandler.readCont` is a plain `var` (no `@Volatile` / atomic). The assignment on the Default worker is not guaranteed to be visible to the dispatch-queue thread that calls `cont.resume(Unit)`; a GC-triggered thread cache invalidation around cycle 13 exposes the stale view and `cont.resume` is skipped. **Workaround in this PR**: reduce the `GC heap size does not grow` cycle count from 50 to 10 so the test stays inside the stable window. The proper fix (make `SuspendBridgeHandler` thread-safe, or route `NwIoTransport` callbacks through a single-thread dispatcher that matches `ioDispatcher`) is tracked in `plan.md` under "テスト安定化 (CI timing races)".
- `IoUringPipelinedServerChannel.start()` now blocks until every worker `EventLoop` has enqueued its multishot accept SQE before returning to the caller of `IoUringEngine.bindPipeline(...)`. Previously the start routine dispatched the accept-SQE submission asynchronously via `IoUringEventLoop.dispatch(...)` and returned immediately, so the kernel's `SO_REUSEPORT` hashing could route the very first incoming connection to a worker whose accept SQE had not yet been submitted; the connection then sat in that worker's listen queue until the worker thread next ran its drain-tasks loop. Under CPU contention on GitHub Actions runners (4 vCPUs), the wait could exceed the client-side `SO_RCVTIMEO=5s` and produce spurious `IoUringPipelinedServerTest.pipelined echo works with acceptDirectAlloc enabled` failures (observed once on PR #304 CI; rerun recovered). The new implementation creates one `CompletableDeferred<Unit>` per worker, completes it on the worker thread after `submitMultishot` returns, and `runBlocking { ... .await() }` on all of them before `start()` returns. `submitMultishot` failures are rethrown on the calling thread via `completeExceptionally`, so a `PipelinedServer` is never returned in a half-armed state. The barrier waits only for the SQE to be enqueued in userspace, not for the kernel to observe it — sufficient in practice because the dispatched `Runnable` runs inside the EL's `drainTasks` phase, which is followed immediately by `io_uring_submit_and_wait` on the same loop iteration.
- `IoUringPipelinedServerTest` helper `rawWrite` now loops until every byte has been written (previously only asserted `n > 0`, tolerating short writes silently) and `rawRead` now fails loudly on EOF / `SO_RCVTIMEO` instead of returning a partial payload (which used to mask the real cause of assertion mismatches as a bare `kotlin.AssertionError at null:-1`).
- `./gradlew assemble` (and any task that cascades through `compileLinuxMainKotlinMetadata` / `compileKotlinLinuxX64` / `compileKotlinMacosX64` on a mismatched host) now succeeds on both macOS and Linux hosts. Four structural issues were fixed together: (1) `:keel-engine-epoll` / `:keel-engine-io-uring` / `:keel-engine-kqueue` / `:keel-engine-nwconnection` were included in the build on every host but declare only platform-specific native targets with cinterops that require the matching OS toolchain; their `compileLinuxMainKotlinMetadata` / `compileMacosMainKotlinMetadata` fired even where the cinterop klib could not be produced, causing `Unresolved reference` errors for `posix_inet` / `epoll` / `kqueue` / `keel_eventfd_*` / `keel_writev`. These four modules are now host-gated in `settings.gradle.kts` so they are excluded from the build on a mismatched host — Linux-only modules only on Linux hosts, macOS-only modules only on macOS hosts. (2) `:keel-ktor-engine`, `:keel-tls-openssl`, `:keel-tls-mbedtls`, `:keel-tls-awslc` referenced the host-gated engine projects in their `macosMain` / `linuxMain` / `macosTest` / `linuxTest` source-set blocks, which evaluate at configuration time regardless of target declaration. Those `project(":keel-engine-*")` references are now wrapped in the same host probe so they only evaluate on a matching host. (3) The pre-existing `afterEvaluate` block in root `build.gradle.kts` that disables cross-architecture cinterop tasks (`cinterop*LinuxX64` on `macosArm64`, etc.) left the matching `compileKotlin<target>` / `<target>MainKlibrary` tasks enabled, so they ran and failed because their cinterop dependency had been silently disabled. The disable filter is widened to cover every task name containing the target token (`LinuxX64`, `LinuxArm64`, `MacosX64`, `MacosArm64`), so the whole per-target task family is coherently skipped. (4) Intermediate `linuxMain` / `macosMain` source-set names were initially covered by the same filter but that broke `dokkaGenerateModuleHtml` on cross-platform modules like `:keel-tls` (which legitimately needs the intermediate metadata transforms for cross-source-set Dokka rendering), so the filter only targets the concrete target tokens. Net result: `./gradlew assemble` / `./gradlew -Ptls -Pbenchmark assemble` both succeed on a single host.
- `./gradlew dokkaGeneratePublicationHtml` now succeeds on a clean `main`. Previously `:keel-core:compileCommonMainKotlinMetadata` failed because `expect object SystemDnsResolver : DnsResolver` did not declare `resolve(hostname, hints)` in the expect body — the KMP commonMain metadata compiler requires the contract to be visible without peeking at platform actuals. The expect object now declares `override suspend fun resolve(hostname: String, hints: ResolveHints): ResolverResult`, and the JVM / JS / Native actuals mark their implementation `actual override`.
- `./gradlew -Ptls -Pbenchmark dokkaGeneratePublicationHtml` now succeeds on both macOS and Linux hosts. Three structural issues were fixed together: (1) `keel_fill_sockaddr_un` in `keel-native-posix/cinterop/posix_socket.def` took `struct sockaddr_un *` — a type whose ABI layout differs between Linux (`sun_path[108]`) and macOS (`sun_path[104]`), so the function signature could not be commonized at the `nativeMain` source set and `compileNativeMainKotlinMetadata` failed with `Unresolved reference 'keel_fill_sockaddr_un'`. The signature now takes `struct sockaddr_storage *` (ABI-identical across platforms per RFC 3493) with an internal cast, restoring commonization without changing Kotlin call sites (they already passed `sa.reinterpret()` through an opaque pointer). (2) `:benchmark` declared `macosArm64` / `linuxX64` executable targets unconditionally; on a macOS host, Gradle then tried to resolve `:benchmark:linuxX64CompileKlibraries` against `:keel-ktor-engine`, which is host-gated and does not publish a `linux_x64` variant on a mac host, producing a cryptic "no matching variant" error under `-Pbenchmark`. Native `:benchmark` targets are now host-gated via the same `hostOs` probe used in `:keel-ktor-engine`, so variant resolution is consistent across hosts. (3) `PipelineHttpRoutes.kt` lives in `:benchmark:commonMain` and imports `keel-codec-http` symbols, but the `keel-codec-http` dependency was declared only on per-target source sets (`jvmMain` / `jsMain` / `macosMain` / `linuxMain`), so `:benchmark:compileCommonMainKotlinMetadata` failed with `Unresolved reference 'codec'`. The dependency is moved to `commonMain` and the redundant per-target entries are removed.

### Added

- **Phase 11**: Native TCP engines (`KqueueEngine` / `EpollEngine` / `IoUringEngine`) gain IPv6 support end-to-end. `keel-native-posix` no longer hardcodes `AF_INET` / `inet_pton(AF_INET, …)`; `PosixSocketUtils.createServerSocket` / `createReusePortServerSocket` / `createUnconnectedSocket` / `connectNonBlocking` all accept an `IpAddress` and branch internally between `sockaddr_in` / `AF_INET` (`IpAddress.V4`) and `sockaddr_in6` / `AF_INET6` (`IpAddress.V6`). The `sin6_addr` union is never touched from Kotlin — new `posix_socket.def` helpers (`keel_init_sockaddr_in6` / `keel_fill_sockaddr_in6_addr` / `keel_extract_sockaddr_in6_addr` / `keel_sockaddr_family`) fill the struct via `memcpy` on the first 16 bytes, sidestepping the Linux glibc (`__in6_u`) vs. macOS (`__u6_addr`) union field-name incompatibility. `getLocalAddress` / `getRemoteAddress` read back through `sockaddr_storage` + `keel_sockaddr_family` so they auto-detect V4 vs. V6. `InetSocketAddress.requireIp()` is a new `keel-core` helper returning the parsed `IpAddress` for synchronous call sites that need the V4 / V6 distinction (e.g. `bindPipelineInet` choosing the socket family). Regression coverage: new `IPv6 loopback bind connect echo round trip` test on each of the three Native engines binds to `::1`, connects, and round-trips a payload.
- **Phase 11**: `UnixSocketAddress` now carries a kernel-aware `@prefix` convention for Linux abstract-namespace Unix sockets, matching the display form used by `ss` / `netstat` / `systemd` (`ListenStream=@myapp.sock`). `UnixSocketAddress("@myapp")` and `UnixSocketAddress("\u0000myapp")` both resolve to the same kernel endpoint — the former is human-readable and safe to embed in config files, the latter matches Go / Netty / Python input conventions. New properties: `isAbstract` detects either form; `kernelPath` renders the `\u0000`-prefixed string the kernel expects. `toString()` emits `unix:@name` for abstract and `unix:/path` for filesystem, matching ss / netstat output. Explicit factories `UnixSocketAddress.filesystem(path)` / `UnixSocketAddress.abstract(name)` are the recommended API; the one-argument constructor still works for config-file deserialisation. Equality stays field-wise on `path`, so `@foo` and `\u0000foo` compare unequal as data classes while sharing a `kernelPath` — prefer the factories to avoid the ambiguity.
- **Phase 11 PR C-3**: `NodeEngine` (JS) and `NwEngine` (Apple Network.framework) now support `UnixSocketAddress` for `bind` / `connect` / `bindPipeline`. **NodeEngine** dials via Node.js `net.createServer().listen({ path })` / `net.createConnection({ path })`; filesystem paths work on every platform Node runs on, Linux abstract-namespace (`@prefix` / `\u0000` prefix) works on Linux (Node.js forwards the leading NUL to the kernel) and throws `UnsupportedOperationException` early on macOS / Windows via a runtime `process.platform` check, matching the engine contract established for kqueue. `Net.createConnection(options: dynamic)` is added to the Kotlin external declarations for path-based client dial. **NwEngine** constructs a filesystem UDS endpoint via the public `nw_endpoint_create_address(sockaddr *)` API — a `sockaddr_un` is filled in the `keel_nw_endpoint_create_unix` C wrapper and passed through to Network.framework, avoiding the exported-but-undeclared SPI `nw_endpoint_create_unix` symbol (SPI usage could be flagged during App Store review). Listener binding follows the pattern documented by Apple DTS on [developer.apple.com/forums/thread/756756](https://developer.apple.com/forums/thread/756756): `nw_parameters_set_local_endpoint(params, endpoint)` on plain TCP parameters + `nw_listener_create(params)` (no-port variant). Darwin's `sun_path[104]` limit (incl. NUL) is validated up-front via `keel_nw_unix_path_max`, so paths are rejected with `IllegalArgumentException` at the engine boundary rather than failing deep inside Network.framework. Abstract-namespace addresses are rejected because macOS / iOS kernels do not implement the Linux abstract namespace. Listener-level TLS over UDS is rejected for `bindPipeline` (the threat model does not fit UDS).
- **Phase 11 PR C-2**: JVM engines (`NioEngine` / `NettyEngine`) now support `UnixSocketAddress` for `bind` / `connect` / `bindPipeline` via Java 16+ `ServerSocketChannel.open(StandardProtocolFamily.UNIX)` + `UnixDomainSocketAddress.of(Path.of(path))` (NIO) and Netty 4.2's `NioServerDomainSocketChannel` / `NioDomainSocketChannel` (Netty). Filesystem paths only — the JDK's `UnixDomainSocketAddress` API does not accept leading `\u0000` / `@prefix`, so abstract-namespace addresses throw `UnsupportedOperationException` at the engine boundary (same contract as `KqueueEngine` on macOS). `NioPipelinedChannel.toSocketAddress` / `NettyPipelinedChannel.toSocketAddress` now emit a keel `UnixSocketAddress` when the underlying Java socket address is a `UnixDomainSocketAddress`, so `ch.localAddress` / `ch.remoteAddress` round-trip correctly on UDS channels.
- **Phase 11 PR C-1**: Native engines (`KqueueEngine` / `EpollEngine` / `IoUringEngine`) now support `UnixSocketAddress` for `bind` / `connect` / `bindPipeline`. Filesystem sockets work on macOS and Linux; Linux abstract sockets (`\u0000` / `@prefix`) work on epoll + io_uring. kqueue rejects abstract addresses with an early `UnsupportedOperationException` because macOS does not implement the abstract namespace. `keel-native-posix` gains `createUnixServerSocket(address, backlog)` / `createUnixUnconnectedSocket()` / `connectUnixNonBlocking(fd, address)` backed by new C wrappers (`keel_bind_un` / `keel_connect_un` / `keel_fill_sockaddr_un`) that compose `struct sockaddr_un` with the correct per-platform `sun_path` capacity (108 bytes Linux / 104 bytes macOS) and the abstract-namespace `addrlen` formula (`offsetof(sun_path) + 1 + name_len`, no trailing NUL). `io_uring` connects via `IORING_OP_CONNECT` on an async-safe `struct sockaddr_un` byte buffer so the native struct type does not need to be bound in cinterop. `bindPipeline` on io_uring uses a single server fd (SO_REUSEPORT is not supported on AF_UNIX) instead of the per-worker REUSEPORT fan-out used for TCP — acceptable for typical IPC / sidecar workloads.
- **Phase 11**: `CachingDnsResolver(delegate, ttl = 30.seconds, maxSize = 1024, timeSource = TimeSource.Monotonic)` in `keel-core`. Opt-in wrapper around any `DnsResolver` that caches successful resolutions for [ttl] and evicts least-recently-used entries beyond [maxSize]. The default `IoEngineConfig.resolver = DnsResolver.SYSTEM` stays OS-delegating (NSS / nscd / systemd-resolved); wrap with `CachingDnsResolver` only when the hot path resolves the same hostnames repeatedly — e.g. `IoEngineConfig(resolver = CachingDnsResolver(DnsResolver.SYSTEM, ttl = 30.seconds))`. **Positive caching only**: failed lookups are not cached, so a transient DNS outage does not lock future retries. Negative caching is deferred until a concrete client use case justifies the second TTL; the constructor stays source-compatible (fourth parameter `timeSource` is test-facing only). Cache entries store the upstream family-neutral result and `resolve(hints)` re-applies the `FamilyPreference` filter per call, so one fetch serves subsequent `V4Only` / `V6Only` / `Any` queries. **Single-flighted via a decoupled scope**: concurrent misses for the same hostname await one shared `Deferred<ResolverResult>` produced by a `SupervisorJob`-backed internal `CoroutineScope`, so cancellation of any individual caller does not cancel the upstream fetch and does not propagate cancellation to unrelated joiners. An HTTP / RPC client that fires many coroutines at cold start therefore sees one `getaddrinfo` call serving everyone, and a single caller's cancellation (e.g. a per-request timeout) no longer disrupts others resolving the same backend. When the upstream fetch fails the in-flight entry is dropped and the failure is broadcast to joiners; the next caller starts a fresh attempt. `close()` cancels the internal scope for graceful shutdown. `invalidate(hostname)` / `invalidateAll()` drop cached entries on demand. Thread-safe via an internal `Mutex`.
- **Phase 11**: `InetSocketAddress.connectWithFallback(resolver, hints, attempt)` extension in `keel-core`. Resolves the address, invokes [attempt] on each candidate `IpAddress` in the order the resolver returned them, returns the first successful result. When every candidate fails, the last error is thrown with all prior errors attached as suppressed. `CancellationException` is re-thrown immediately so caller cancellation always wins over fallback iteration. This is sequential fallback only — candidates run one at a time. RFC 8305 Happy Eyeballs (concurrent V4/V6 racing with staggered start) stays out of scope for engine-level connect and will be added in the Phase 12+ HTTP Dialer layer, where it can compose multiple `engine.connect(ip, port)` attempts in parallel without bloating the engine interface.

### Changed

- **BREAKING (Phase 11)**: `keel-native-posix` removes `POSIX_IPV4_RESOLVE_HINTS` and `InetSocketAddress.resolveForPosixSocket(resolver)`; the V4Only coercion was a workaround for the old `AF_INET`-only socket path and is no longer needed now that Native engines bind/connect over IPv6 natively. Callers that held those symbols should use `InetSocketAddress.resolveFirst(resolver)` (or `connectWithFallback`) with the default `ResolveHints.DEFAULT` (`FamilyPreference.Any`). `PosixSocketUtils` public surface also changes: `createServerSocket(host: String, port, backlog)` and siblings are replaced by `(address: IpAddress, port, backlog)` overloads; `createUnconnectedSocket()` now takes an `IpAddress` family marker (any V4 / V6 instance selects the family). `InetSocketAddress.resolveForPosixSocket` and the V4Only hint coercion previously used by `kqueue` / `epoll` / `io_uring` are gone.
- **Phase 11**: `KqueueEngine` / `EpollEngine` / `IoUringEngine` / `NioEngine` / `NettyEngine` now route `connectInet` through `InetSocketAddress.connectWithFallback`, so a hostname that resolves to multiple candidates (e.g. a CDN round-robin, or `localhost` with both `127.0.0.1` and `::1` under dual-stack) tries each one in order and returns on the first successful connect. Native engines no longer force `FamilyPreference.V4Only`; `connectWithFallback` receives the resolver's native ordering (V4 + V6 mixed) and each candidate drives the matching `sockaddr_in` / `sockaddr_in6` path. JVM engines accept any family the resolver returns. `NodeEngine` and `NwEngine` are unchanged — their platform connect APIs already handle multi-candidate fallback at the OS layer, so duplicating it in keel would race against the OS-driven retry. Full candidate exhaustion throws the last error with the earlier ones attached as suppressed exceptions so diagnostics are preserved.
- **Phase 11**: Native `SystemDnsResolver` now backs `DnsResolver.resolve(hostname, hints)` with `getaddrinfo(3)` instead of the Phase 11 A-2 stub. Blocking calls run on `Dispatchers.Default` (`Dispatchers.IO` is `internal` on Native in kotlinx-coroutines 1.10); for production workloads wrap the resolver with `CachingDnsResolver` (Phase 11 PR D) so the blocking path is rarely hit. IP-literal hostnames short-circuit via `IpAddress.parseOrNull` and never enter the blocking path. `ResolveHints.canonicalName` populates `ResolverResult.canonicalName` via `AI_CANONNAME`. IPv6 addresses are extracted via raw 16-byte `sin6_addr` reads plus `sin6_scope_id`, sidestepping the platform-specific `in6_addr` union field name (`__in6_u.__u6_addr8` on Linux vs. `__u6_addr.__u6_addr8` on macOS). `gai_strerror` wraps `getaddrinfo` failure codes into a `RuntimeException` with the descriptive message. Phase 11 A-2 callers that caught `UnsupportedOperationException` for hostnames should migrate to `RuntimeException` (the `IoUringEngineTest.connect to invalid host address throws` expectation was updated in this release).
- **Phase 11**: Native engines (`KqueueEngine` / `EpollEngine` / `IoUringEngine`) now resolve `InetSocketAddress` through the new `InetSocketAddress.resolveForPosixSocket(resolver)` helper in `keel-native-posix`, which forces `FamilyPreference.V4Only`. Rationale: the Native socket path (`PosixSocketUtils` / `inet_pton(AF_INET, ...)`) is IPv4-only, so a hostname like `localhost` that maps to both `127.0.0.1` and `::1` in `/etc/hosts` would otherwise yield an IPv6 first-hop that the IPv4 socket rejects. Will be relaxed once the Native socket path gains IPv6 support.
- **BREAKING (Phase 11)**: `StreamEngine.bind` / `connect` / `bindPipeline` now take a `SocketAddress` (the sealed hierarchy introduced in PR #294) instead of a raw `host: String, port: Int` pair. The old `(host, port)` form is preserved as a default interface method that simply builds an `InetSocketAddress(host, port)`, so existing call sites continue to compile. `UnixSocketAddress` is accepted by the signatures but every engine throws `UnsupportedOperationException` until Phase 11 PR C wires UDS through. Non-suspend `bindPipeline` requires IP literals — hostnames throw `UnsupportedOperationException` via the new `InetSocketAddress.requireIpLiteral()` helper because synchronous call sites cannot invoke a suspending resolver. Suspending `bind` / `connect` paths resolve `Host.Name` via `IoEngineConfig.resolver.resolve(...)`; today's resolver wires `InetAddress.getAllByName` (JVM), `dns.lookup({ all: true })` (JS), and — until Phase 11 PR B lands `getaddrinfo` on Native — a stub that rejects hostnames on Native targets. Engines pick the first resolved `IpAddress` (sequential fallback for multi-IP hostnames is a follow-up). Migration for callers holding a prebuilt `SocketAddress`: pass it directly; no explicit conversion needed. Migration for callers holding `(host, port)`: no source change.
- **BREAKING (Phase 11)**: `IoEngineConfig` grows a `resolver: DnsResolver` field (default `DnsResolver.SYSTEM`). `DnsResolver` is a new interface in `keel-core` with `suspend fun resolve(hostname, hints): ResolverResult`, along with `ResolveHints` (family filter, canonical-name flag, optional `timeout`), `FamilyPreference` (`Any` / `V4Only` / `V6Only`), and `ResolverResult(addresses, canonicalName)`. Replace the resolver via `IoEngineConfig(resolver = CachingDnsResolver(DnsResolver.SYSTEM, ttl = 30.seconds))` once the caching wrapper lands (Phase 11 PR D); the interface is pluggable so hickory-dns / c-ares / HTTPS-DoH implementations can be added without re-touching engines. `InetSocketAddress.resolveAll(resolver)` / `resolveFirst(resolver)` extensions expose the resolve step so other code paths (e.g. Happy Eyeballs in a future HTTP client) can share the logic. `SystemDnsResolver` is implemented as an `expect object` with platform actuals: JVM runs `InetAddress.getAllByName` on `Dispatchers.IO`; JS wraps Node.js `dns.lookup`; Native rejects hostnames pending PR B.
- **BREAKING (Phase 11)**: `SocketAddress` is now a `sealed class` with two subclasses — `InetSocketAddress(host: Host, port: Int)` and `UnixSocketAddress(path: String)`. `Host` is itself a sealed hierarchy (`Host.Name(value)` / `Host.Ip(IpAddress)`), and `IpAddress` is a new sealed hierarchy (`V4(Int)` / `V6(ULong, ULong, scopeId: Int)`) with a pure-Kotlin RFC 5952 parser (`IpAddress.parseOrNull` / `parse` / `ofBytes`) and canonical string form (V4 dotted-decimal, V6 RFC 5952 compressed + `%scope`). `InetSocketAddress` accepts a string-form constructor `InetSocketAddress(hostOrIp: String, port: Int)` that parses IP literals (IPv4, IPv6 with optional brackets or `%scope` suffix) eagerly into `Host.Ip` and keeps anything else as `Host.Name` for later resolution (`isUnresolved` reflects the distinction, matching Java `InetSocketAddress.isUnresolved`). Existing call sites `SocketAddress(host, port)` must migrate to `InetSocketAddress(host, port)` — the data-class form of `SocketAddress` is removed. `toString()` uses `[::1]:8080` bracket form for IPv6, `host:port` for V4/hostname, and `unix:/path/to/sock` for UDS. A `hostString` convenience property on `InetSocketAddress` returns the canonical host form (hostname or RFC 5952 IP literal). This is the first PR in the Phase 11 SocketAddress redesign; subsequent PRs add `DnsResolver` / `IoEngineConfig.resolver`, migrate `bind/connect` signatures to accept `SocketAddress`, introduce UDS and `DatagramEngine`. See `design.md` §29 for the full plan.
...
- keel-core: `IpAddress.V4` / `IpAddress.V6` `toString()` now returns the RFC 5952 canonical form (`"127.0.0.1"` / `"::1"`) instead of the `data class`–auto-generated `V4(value=...)` / `V6(high=..., low=..., scopeId=...)` string. The sealed parent declared `override fun toString(): String = toCanonicalString()`, but Kotlin data classes still auto-generated their own `toString` which shadowed it; explicit `override fun toString() = toCanonicalString()` on both V4 and V6 suppresses the auto-generation. Callers using `"$ip"` / `"host:$port"` string interpolation — including engine log messages and tests — now see the intended canonical form. Pure fix; caught while un-blocking `keel-core:jvmTest` in the `commonTest` cleanup.
- keel-core, keel-engine-nio, keel-engine-netty, keel-engine-kqueue, keel-engine-epoll, keel-engine-io-uring, keel-engine-nwconnection: `IoTransport.close()` is now thread-safe on every engine. Previously the `opened` flag in `AbstractIoTransport` was plain `var` (no volatile / no atomic), and each engine's `close()` flipped the flag, drained `pendingWrites`, and released the native resource without any serialisation with the EventLoop-side read / write / flush path. Concurrent callers could both pass the `if (!opened)` check and release the same `IoBuf` twice, and a close from a non-EventLoop thread could race an in-flight write on the EventLoop. Fix: promote `AbstractIoTransport.opened` to `@Volatile` so `isOpen` stays lock-free; each `close()` now either runs on the owning EventLoop (NIO / Netty / kqueue / epoll / io_uring) or dispatches teardown onto it, with a re-check of `opened` inside the teardown task to absorb concurrent callers into a single idempotent run. `IoUringIoTransport` was already on this pattern; the re-check inside `teardownOnEventLoop` closes the remaining race window where two callers both passed the outer `if (!opened)` read. `NwIoTransport` now takes the per-connection serial `dispatch_queue_t` through its constructor and dispatches the teardown onto that queue, which is the same queue every NWConnection read / write completion callback runs on; this gives a total order between close and in-flight callback mutations of `pendingWrites`, matching the other transports' EventLoop-dispatch contract. `NodeIoTransport` stays unchanged (JS single-thread), with its `Thread safety` KDoc section updated to match the multi-threaded engines' contract.
- keel-engine-nio, keel-engine-kqueue, keel-engine-epoll, keel-engine-io-uring: `Server.close()` is now thread-safe on every native / JVM engine. Previously `NioServer`, `KqueueServer`, `EpollServer`, and `IoUringServer` kept `_active` and `pendingAcceptCont` as plain (non-volatile, non-synchronised) fields while their KDoc invariants required callers to hold the boss EventLoop thread. Production code in `KeelApplicationEngine` routinely calls `server.close()` from the `serverJob` coroutine on `Dispatchers.Default`, so the invariant was being violated under structured-concurrency shutdown. Fix: `NioServer` guards state transitions with a `synchronized(lock)` block (Netty-style); `KqueueServer` / `EpollServer` use a `pthread_mutex_t` helper (NwServer-style); `IoUringServer` latches close via `kotlin.concurrent.AtomicInt` and dispatches the multishot / `pendingFds` / `pendingAcceptCont` cleanup onto the boss EventLoop so every ring-scoped call stays on its owning pthread. `NodeServer` KDoc is updated to reflect the JS single-thread invariant. `@Volatile` is applied to `_active` so `isActive` reads stay lock-free. Concurrent `close()` callers now observe an idempotent transition, and close from non-EL threads no longer races with the accept readiness callback. Covered by `NioServerCloseThreadSafetyTest` (16-way concurrent close; close during suspended accept).
