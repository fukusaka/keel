...
- `benchmark`: new streaming HTTP endpoints `POST /upload-stream` (drains the request body via the engine's read channel, replies with the byte count) and `GET /sse-stream?count=N&size=M` (emits N SSE-style frames of M bytes via chunked Transfer-Encoding) added to every benchmark engine — Ktor `BenchmarkModule` (covers `ktor-keel-*` Pattern B / `ktor-cio` / `ktor-netty`), raw `PipelineHttpRoutes` (covers `pipeline-http-*`), `NettyRawEngine`, `SpringEngine` (Spring WebFlux functional routing), `VertxEngine` (Vert.x web router). Lets the engine's request-body / response-body streaming paths be benchmarked independently of the existing `/hello` + `/large` baseline (#394)
- `benchmark`: WebSocket echo endpoint `GET /ws-echo` added — Ktor route uses `ktor-server-websockets` plugin (covers `ktor-cio`, `ktor-netty`, and `ktor-keel-*` Pattern B once `respondUpgrade` lands), `NettyRawEngine` adds `WebSocketServerProtocolHandler` + a `WebSocketEchoHandler`, `SpringEngine` adds a `WebSocketHandler` + `SimpleUrlHandlerMapping`, `VertxEngine` adds an HTTP-server-level `webSocketHandler`. Pattern B (`ktor-keel-*`) engines reject the upgrade until `respondUpgrade` support lands; non-keel engines (`ktor-cio` / `ktor-netty` / `netty-raw` / `spring` / `vertx`) work today (#394)
- `benchmark`: k6 scenarios `benchmark/k6/upload.js`, `sse.js`, and `ws-echo.js` driving the new endpoints, plus `bench-stream-one.sh` helper that mirrors `bench-one.sh`'s contract (start server, run client, parse output, emit `<name>|<rps>|<p50>|<p99>` row) but invokes k6 instead of wrk. The script picks the right k6 metric per scenario (`http_reqs` / `http_req_duration` for HTTP, `ws_msgs_received` / `ws_ping` for WebSocket — the latter populated by `socket.ping()` calls every `PING_INTERVAL` echoes for ns-precision RTT via Go-side `time.Now()`). README under `benchmark/k6/` documents install + run (#394)
- `benchmark`: `bench-stream-one.sh` `BENCH_K6_SUCCESS_THRESHOLD` env (default 95) parses k6's `checks_succeeded` line and reports `engine||checks=NN.NN%|-` instead of phantom RPS when responses are corrupt (e.g. a chunked-encoder bug failing 99.97% of body-size checks). Raw k6 output now persists under `benchmark/results/{host}/{engine}-{scenario}-{vus}vu-{dur}-{ts}-run{n}.txt` mirroring the wrk raw-output convention so post-mortem audits don't require re-running (#394)
