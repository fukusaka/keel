...
- `server-ktor-base`: `KeelCompressionPlugin` (`KeelCompression`) — Native-only Ktor `RouteScopedPlugin` providing response compression for KMP Native targets, where ktor-server-compression is JVM-only and stock ktor `GZipEncoder` / `DeflateEncoder` are identity no-op stubs. API mirrors ktor's `Compression` plugin (`gzip()`, `deflate()`, `condition { }`, `minimumSize`, `matchContentType`, `excludeContentType`). Backed by `KeelGZipEncoder` / `KeelDeflateEncoder` — `ContentEncoder` adapters routing through `keel-compression-zlib` (libz cinterop). JVM users continue to use `install(Compression)` from ktor-server-compression unchanged
- `benchmark`: install `KeelCompression` on the four Native `KeelCio*` engines (cio-keel-kqueue / -nwconnection / -epoll / -io-uring), closing the response compression gap for ktor-http-cio's raw byte-channel parser path. `Keel*` engines continue to use the engine-pipeline `CompressionHandler` from PR #494 because that path avoids the channel ↔ ByteArray bridge overhead. `BENCH_COMPRESSION_STRICT=true` now passes on all eight Native engines (req/s on macOS: ktor-keel-kqueue 9,353, -nwconnection 8,717, ktor-cio-keel-kqueue 6,864, -nwconnection 6,869, all 0 failures)
