...
- `keel-server-ktor-cio`: new module — Ktor adapter (Pattern C) using `ktor-http-cio`'s parser instead of keel's `:keel-codec-http`. Ships the `KeelCio` `ApplicationEngineFactory` and a `KtorCioConnectionHandler` skeleton. The handler implementation (byte-channel pumps + `parseRequest` / `parseHttpBody` integration + `KeelCioApplicationCall` / Request / Response triple) lands in follow-up PRs (#415)
