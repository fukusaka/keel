...
- `codec-http`: `StaticHeaderTable` process-wide intern for well-known `(name, value)` pairs — 61 HPACK static entries (RFC 7541) + 99 QPACK static entries (RFC 9204) lowercase for H2 / H3 indexed-entry decode, plus 63 HTTP/1.1 Title-Case entries (HPACK + QPACK concrete-value non-pseudo entries Title-Cased + 10 H1 hop-by-hop / framing / cache-busting pairs from RFC 9110 / 9111 / 9112) for the H1 parser hot path. `HttpHeaders.add` reuses the shared `HeaderEntry` on hit (24 B saved). CDN workload N=23 alloc -13.0 % (552 → 480 B/cycle). `hpackStaticEntry(n)` / `qpackStaticEntry(n)` accessors prepare Phase 13 / 14 indexed decode. Production-frequent H1 pairs absent from HPACK / QPACK deferred to a follow-up PR derived from HTTP Archive BigQuery (#592)
