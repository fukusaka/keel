...
- `benchmark`: `NettyRawEngine` SSE handler now flushes per frame (`ctx.writeAndFlush` × N + 1 trailing `writeAndFlush(LAST_CONTENT)`) instead of queueing every frame in `ChannelOutboundBuffer` until the terminator triggered one batch flush; the `netty-raw` SSE bench now matches the per-frame send semantics that `pipeline-http-*` (#440) and `ktor-keel-*` (#441) enforce, so the row is comparable to the keel engines instead of inflating 3-4× via bulk delivery (#TBD)
