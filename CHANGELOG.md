...
- `benchmark`: SSE handlers across `NettyRawEngine`, `zig-bench`, and `rust-bench` now flush per frame to match the per-frame send semantics that `pipeline-http-*` (#440) and `ktor-keel-*` (#441) enforce — `netty-raw` switched per-frame `ctx.write` to `writeAndFlush`, `zig-bench` shrunk its `respondStreaming` buffer to 256 B and added `flush()` per frame, `rust-bench`'s axum `Sse` stream now `yield_now`s between items so hyper can flush each chunk; the SSE rows for these reference servers are now comparable to keel engines instead of inflating 2-4× via batched delivery (#442)
