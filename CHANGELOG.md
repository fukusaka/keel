...
- `engine-io-uring`: honour `IoEngineConfig.idleReadPolicy`. io_uring's multishot `IORING_OP_RECV` is structurally an active-receive operation — peer FIN is delivered as `res = 0` CQE only while a recv SQE is armed — so the data-loss caveat that motivated `IdleReadPolicy` on engine-nio / engine-netty NIO fallback / engine-nwconnection (PR #472) applies here too. With `DETECT_PEER_CLOSE` (default since #475) the engine arms multishot recv at construction via the `IoTransport.onChannelAttached()` hook (PR #475) so peer FIN surfaces through `onReadClosed` regardless of `readEnabled` state. Adds defensive `buf.release()` in the `onCqe` `res > 0` branch when `onRead` is null to prevent provided-buffer-ring slot leak. `IdleReadPolicy` KDoc applicability table updated (engine-io-uring "no" → "yes"); `IoEngineConfig` KDoc adds engine-io-uring to the consulting engines list (#476)
