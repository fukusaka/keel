...
- `server-http`: `UpgradeProtocol` — a connection-takeover hook for protocol upgrades (WebSocket etc.). Registered with `KeelHttpServerBuilder.upgrade(path, protocol)`, resolved by the `Router` (sharing `:name` path matching), and dispatched as the terminal of the middleware chain so middleware runs before the handshake (#537)
