...
- keel-ktor-engine: `KeelApplicationEngine.stop()` now returns promptly in every scenario where the previous implementation blocked for the full `timeoutMillis`. Root cause: `initServerJob`'s body called `ioEngine.close()` as its final statement, which shut down the per-connection EventLoop dispatcher before `handleConnection` children had completed. Children still suspended at that moment (inside `bridge.receiveCatching()` on idle keep-alive reads, or inside an application handler such as `delay(Long.MAX_VALUE)`) could no longer receive cancellation resumption because their dispatcher was gone; `serverJob.join()` therefore could not observe them complete and stop waited for the full `timeoutMillis`. This had a structural quality: it broke the implicit structured-concurrency invariant "a dispatcher outlives every coroutine that runs on it". PR #144 (2026-03-30) worked around the symptom for benchmark servers with a `Runtime.halt(0)` shutdown-hook fallback, but left the underlying dangling children in place. Fix: wait for every child coroutine to complete before closing `ioEngine`. After `stopRequest.join()` and closing the listening sockets, the body enters a `try { children.join() } finally { withContext(NonCancellable) { children.cancelAndJoin(); ioEngine.close() } }` block. Children finish naturally in the graceful path; on grace-period expiry, stop's `serverJob.cancel()` propagates into the children — and because the dispatcher is still alive at that moment, every cancellation lands, every `delay` / `bridge.receiveCatching()` unwinds via `CancellationException`, and `join()` completes. The `NonCancellable` wrapper guarantees `ioEngine.close()` still runs even when the outer job has been cancelled. Regression test `EngineStopLifecycleTest` added: no-client, single idle connection, 20 idle keep-alive connections, and in-flight `delay(Long.MAX_VALUE)` handler all complete under 700 ms (previously up to ~1 s for the idle and in-flight cases).
