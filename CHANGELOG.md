...
- `codec-http`: `StaticHeaderTable` process-wide intern for well-known `(name, value)` pairs (61 HPACK RFC 7541 static entries + ~40 Jetty `HttpParser.CACHE` extensions). `HttpHeaders.add` reuses the shared `HeaderEntry` on hit (24 B saved per hit). CDN workload (N=23) per-cycle alloc -13 % (552 → 480 B); 5/5 intern-hit subset 0 B/cycle. Same table will back HTTP/2 HPACK / HTTP/3 QPACK indexed-entry decode in Phase 13/14 (#592)
