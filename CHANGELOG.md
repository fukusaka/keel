...
- `tls`: `TlsConfig.minVersion` / `maxVersion` (`TlsVersion` enum: `TLS1_2` / `TLS1_3`) pin the negotiable TLS protocol version range portably across all backends — JSSE (`enabledProtocols`), OpenSSL / AWS-LC (`SSL_CTX_set_min/max_proto_version`), MbedTLS (`mbedtls_ssl_conf_min/max_tls_version`). `TlsConfig` rejects an empty range (`minVersion > maxVersion`); a handshake that cannot satisfy the range is aborted. (#633)
