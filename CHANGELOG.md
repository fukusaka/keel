...
- `keel-core`: `Channel.highPriorityIoDispatcher` API for lifecycle-event continuations to bypass the regular `ioDispatcher` queue under load. `SuspendBridgeHandler.read` / `readOwned` use it so the resume from `onInactive` lands in the channel's HIGH queue. Default implementation aliases `ioDispatcher` (passthrough); engines opt in by providing a separate priority queue. Foundation for engine-layer fairness improvements (Phase 1) (#418)
