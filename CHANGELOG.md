...
- `benchmark`: extend `keelNativeCompressionCustomizer` to install `HttpRequestDecompressionHandler` alongside the existing `CompressionHandler` for the four Native `Keel*` engines (kqueue / nwconnection / epoll / io-uring). Closes the inbound compression gap on those engines — the `compression-upload` k6 scenario flips green from `BENCH_COMPRESSION_UPLOAD_STRICT=true` failures to 0%/15-29 K req/s on macOS / Linux. `KeelCio*` follow-up (Native `KeelContentEncodingPlugin`) is the next PR (#498)
