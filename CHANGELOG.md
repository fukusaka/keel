...
- `server-http`: new `keel-server-http` module — a native HTTP server on a keel `StreamEngine`, runnable on every engine target (native / JVM / JS). `KeelHttpServer` start/stop lifecycle wraps Pipeline-mode bind + the HTTP/1.1 server codec; the `keelHttpServer(engine) { }` DSL dispatches each request to one `RouteHandler` via the imperative `call.respond(...)` model, with a `500` guard for handlers that return without responding (#528)
