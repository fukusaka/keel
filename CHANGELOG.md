...
- `io`: `DefaultAllocator.slice()` returns a true zero-copy view sharing the source backing instead of a copy — the JS path now matches Native/JVM, and the documented `slice` contract (shared backing + retained source) holds on every platform (#574)
