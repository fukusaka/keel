 ...
 * Netty engine ([IoEngineConfig.idleTimeoutMillis]). Real sockets and wall-clock
 * timing; the module's [runTest] helper supplies the timeout envelope. The timeout
 * runs at the transport seam ([io.github.fukusaka.keel.pipeline.AbstractIoTransport])
 * backed by [NettyEventLoopTimer] over the channel's Netty `EventLoop`, so it is
 * driven the same way as the wait-loop engines rather than by Netty's own
 * `IdleStateHandler` / `WriteTimeoutHandler`. A minimal echo / big-chunk handler is
 * used rather than the HTTP codec.
 */
class NettyEngineIdleTimeoutTest {
    ...
    fun `a silent client is closed after the idle timeout`() = runTest {
        val engine = NettyEngine(IoEngineConfig(threads = 1, idleTimeoutMillis = IDLE_MS))
    ...
    fun `a client active within the idle window is not closed`() = runTest {
        ...
        val engine = NettyEngine(IoEngineConfig(threads = 1, idleTimeoutMillis = ACTIVE_IDLE_MS))
