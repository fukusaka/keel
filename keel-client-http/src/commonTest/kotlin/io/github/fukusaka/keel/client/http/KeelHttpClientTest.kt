 ...
 * response.
 *
 * The in-memory loopback completes synchronously, but every test is
 * bounded by [withTimeout] as a guard against an accidental hang in the
 * suspend `read` path (the sibling `KeelHttpTestTest` pattern).
 */
class KeelHttpClientTest {
    ...
    @Test
    fun `GET returns the status body and headers`() = runTest {
    ...
    @Test
    fun `POST sends the body and Content-Length`() = runTest {
        withTimeout(asyncBudget) {
            val engine = InMemoryEngine()
            val server = keelHttpServer(engine) {
                connector { host = "127.0.0.1"; port = 0 }
    ...
    @Test
    fun `a query string reaches the server`() = runTest {
        withTimeout(asyncBudget) {
            val engine = InMemoryEngine()
            val server = keelHttpServer(engine) {
                connector { host = "127.0.0.1"; port = 0 }
    ...
    @Test
    fun `a caller-supplied Host header is preserved`() = runTest {
        withTimeout(asyncBudget) {
            val engine = InMemoryEngine()
            val server = keelHttpServer(engine) {
                connector { host = "127.0.0.1"; port = 0 }
    ...
    @Test
    fun `a fresh-connect request leaks no pooled buffers`() = runTest {
        withTimeout(asyncBudget) {
