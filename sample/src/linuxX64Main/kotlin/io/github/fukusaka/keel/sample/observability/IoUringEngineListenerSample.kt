...
/**
 * Visual-verification sample for pluggability item 12 B2.5 step 4:
 * `BufferAllocator.lifecycleListener` propagation into `IoUringEngine`.
 *
 * Native counterpart of `sample/observability/NettyEngineListenerSample`
 * (B2.5 step 2) and `sample/observability/NwEngineListenerSample`
 * (B2.5 step 3) — same surface, different engine. Wires an
 * [AtomicTracker] (a minimal thread-safe
 * [BufferAllocatorLifecycleListener]) into a [SlabAllocator]'s
 * `lifecycleListener` parameter ([SlabAllocator] is the Native-side
 * [io.github.fukusaka.keel.buf.PooledAllocator] subclass —
 * `PooledDirectAllocator` is JVM-only), passes that allocator into an
 * [IoUringEngine] via [IoEngineConfig], then drives a sustained
 * loopback echo through `engine.bind()` / `engine.connect()` — both
 * client and server sides run on the same engine.
 *
 * Each round-trip drives:
 *  - the inbound path that picks a `RingBufferIoBuf` wrapper from the
 *    provided buffer ring on each recv CQE — `reset()` fires
 *    `onAllocated` and the refcount-zero transition in `release()`
 *    fires `onReleased` after the slot is returned to the ring
 ...
 * ./sample/build/bin/linuxX64/IoUringListenerSampleReleaseExecutable/IoUringListenerSample.kexe --iters=2000
 * ```
 *
...
fun main(args: Array<String>) {
    ...
    println("keel IoUringEngine listener-wiring sample (item 12 B2.5 step 4).")
    println("Echoing loopback traffic through IoUringEngine; printing AtomicTracker counters every $SNAPSHOT_INTERVAL_MILLIS ms.")
