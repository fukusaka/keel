...
class TransportPreSplitReportTest {
    ...
    @Test
    fun `a refused send on a transport that reports every end the old way leaves the read a misuse`() =
        runTest(timeout = 15.seconds) {
            val tracker = TrackingAllocator()
            // The base's answer, which every engine here gives — including the one
            // family that raises a refused send at all. No bridge, so this
            // transport's one report closes the channel and the read that follows
            // is the one the mark decides.
            ...
            transport.onConnectionFailure?.invoke(RuntimeException("refused send"))
            transport.onReadClosed?.invoke()

            assertFalse(channel.isOpen, "premise: this transport's one report closed the channel")
            assertFalse(channel.endedByTransport, "this arm takes no mark, as it took none before the split")
            val dst = tracker.allocate(8)
