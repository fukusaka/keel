 ...
 * @param idleTimeoutMillis Per-client override of the idle (no-progress) timeout
 *   for this connection (see [IoEngineConfig.idleTimeoutMillis]). `null` (default)
 *   inherits the engine-wide value; `0` disables it. If non-null, must be `>= 0`.
 *   Currently honoured by the epoll / kqueue engines.
 */
public open class ConnectConfig(
    ...
    public val idleTimeoutMillis: Long? = null,
) {
