...
interface PipelinedChannel : Channel {
    ...
    override suspend fun read(buf: IoBuf): Int {
        ...
        return withContext(ioDispatcher) {
            ...
            // not arming costs the connection — enabling the read is what
            // starts the read-idle clock on every engine here, and the only
            // way a Coroutine-mode caller starts one that is not running
            // already. That clock is what reclaims a descriptor after the
            // peer finished and its caller never closed.
