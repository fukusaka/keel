...
abstract class AbstractIoTransport(
     ...
     * reclamations in this class, each engine's own peer-close handling and
     * the failure handling of netty, io_uring and the readiness transports
     * all route through this one gate (nio reports nothing on a failed read
     * or write; NWConnection logs a failed send and relies on its receive
     * side). A read side closes once; a second report would tell a listener
     * that already acted on the first — a parked reader woken with EOF, a
     * bridge that stopped expecting data — the same thing again. A pipeline
     * absorbs a repeat, but that is a guard in another module; a
     * Coroutine-mode caller has none. A subclass that invokes [onReadClosed]
     * directly steps outside the contract [IoTransport.onReadClosed] states.
     *
    ...
    private fun reportReadClosedNow() {
        ...
        // why the peer finishing is not a reason to stop reclaiming. A throw
        // from the listener is not contained here: it travels back on
        // whatever path made the report, except the idle reclamation, which
        // carries it and closes regardless.
        onReadClosed?.invoke()
    }
     ...
     * every engine routes its endings through [reportReadClosedOnce], which
     * is where the arm below sends this one too, and what that one gate
     * covers is written there.
     *
     * **EventLoop thread**, like every other wind-down step.
     */
    ...
    /**
     * Whether the read side's report has gone out — through
     * [reportReadClosedOnce], or through [reportEndOnce] on a transport that
     * still makes one report for every end, where the end takes that same
     * hook. It is therefore not by itself evidence that the peer finished:
     * that is this flag with [endAlreadyReported] still `false`. For a
     * transport deciding what a
     * later event means: a receive on a stream already declared complete, or
     * a close the platform performs once both halves have ended, is not a
     * second thing to report when the read side's report already went.
     */
