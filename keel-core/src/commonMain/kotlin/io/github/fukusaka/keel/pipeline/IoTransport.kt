 ...
 * this tree but the doubles its own tests define — reports every one of those
 * endings on [onReadClosed], the second line included; see
 * [reportsEveryEndAsReadClosed].
 *
 * ```
 ...
 * otherwise noted.
 *
 * Almost a pure interface: the defaults here exist so an implementor
 * written before a member was added still compiles, and each says what
 * keeping it gives up. Use [AbstractIoTransport] for shared behaviour
 * (awaitPendingFlush, awaitClosed, callback properties).
 *
 * ## Read Path
 *
...
interface IoTransport {
     ...
     * half-closed. The transport does NOT call [close] — the callback owner
     * decides whether the whole connection ends.
     *
     * At most once per transport, whichever paths observe it: a peer FIN,
     * the idle reclamation, a read re-armed after the FIN that reads the
     * same end again, and — while a transport still answers `true` to
     * [reportsEveryEndAsReadClosed] — every other ending too, all route
     * through one gate ([AbstractIoTransport.reportReadClosedOnce]; an
     * implementation outside that class owes the same rule), so the listener
     * never hears the same end twice and need not guard against it.
     *
     ...
     * listener (one can still be answered, the other cannot).
     *
     * Unless the transport answers `true` to [reportsEveryEndAsReadClosed],
     * which every transport in this tree does but the doubles its own tests
     * define — for one of those this is
     ...
     *
     * The default accessors store nothing, so a transport written before this
