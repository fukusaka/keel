...
/**
 * Detects `IoBuf(size)` allocations that are not protected by
 ...
 * may internally release the buf). Use `@Suppress("IoBufLeak")` for
 * intentional ownership transfers documented in KDoc.
 */
class IoBufLeakRule(config: Config) : Rule(config) {

    override val issue = Issue(
        "IoBufLeak",
        Severity.Defect,
        "IoBuf allocated without try-finally release/close protection",
        Debt.TEN_MINS,
    )
    ...
    override fun visitCallExpression(expression: KtCallExpression) {
        ...
        if (callee != "IoBuf") return

        // Skip allocator implementations — they return IoBuf to caller.
        // Also skip test functions (test helper methods that allocate buffers).
        // Skip allocator implementations — they return IoBuf to caller.
        // Single-expression functions (fun foo() = IoBuf(...)) have the
        // IoBuf call as the body expression, so strict=false matches.
        ...
        if (!hasProtectedRelease) {
            report(
                CodeSmell(
                    ...
                    "IoBuf allocated without try-finally release/close. " +
                        "Use try { ... } finally { buf.release() } or " +
                        "@Suppress(\"IoBufLeak\") for intentional ownership transfer.",
                ),
            )
        }
    }
