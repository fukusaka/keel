...
import awslc.SSL
import awslc.SSL_ERROR_NONE
import awslc.SSL_ERROR_WANT_READ
import awslc.SSL_ERROR_WANT_WRITE
import awslc.SSL_ERROR_ZERO_RETURN
import awslc.SSL_do_handshake
import awslc.SSL_free
import awslc.SSL_get_error
import awslc.SSL_is_init_finished
...
/**
 * [TlsCodec] implementation backed by AWS-LC (BoringSSL fork) with pointer-based BIO.
 *
 * Uses a custom [BIO_METHOD][awslc.BIO_meth_new] with recv/send callbacks
 * that read from and write to caller-owned [IoBuf] memory directly — same
 * zero-copy approach as [OpenSslCodec] and Mbed TLS's keel_mbedtls_bio_ctx.
 *
 ...
 */
class AwsLcCodec internal constructor(
    private val ssl: CPointer<SSL>,
    private val bioCtx: keel_awslc_bio_ctx,
) : TlsCodec {
    ...
    override val negotiatedProtocol: String?
        get() = memScoped {
            ...
            val data = keel_awslc_get_alpn(ssl, lenVar.ptr)
    ...
    override val peerCertificates: List<ByteArray>
        get() = emptyList() // Deferred: peer cert extraction requires i2d_X509.
    ...
    override fun unprotect(ciphertext: IoBuf, plaintext: IoBuf): TlsCodecResult {
        ...
        // of protect(), not the plaintext output here. If AWS-LC needs
    ...
    private fun tlsError(op: String, sslError: Int): TlsException {
        val reason = keel_awslc_err_string()?.toKString() ?: "unknown"
        return TlsException(
            "AWS-LC $op failed: SSL_ERROR=$sslError ($reason)",
