...
class AwsLcCodecFactory : TlsCodecFactory {
    ...
    /**
     * Pins the negotiable protocol version range. Both bounds are set
     * explicitly: the floor ([TlsConfig.minVersion], default TLS 1.2) so
     * SSLv3 / TLS 1.0/1.1 are never negotiable, and the ceiling
     * ([TlsConfig.maxVersion] or [TlsVersion.TLS1_3] when null) so keel
     * never negotiates a version it does not enumerate. Range validated by
     * [TlsConfig]. (AWS-LC honours the version range; it cannot restrict
     * TLS 1.3 cipher suites, but that is unrelated to version pinning.)
     */
    private fun configureProtocols(ctx: CPointer<SSL_CTX>, config: TlsConfig) {
        ...
        if (keel_awslc_set_max_proto_version(ctx, awslcVersion(max)) != 1) {
            SSL_CTX_free(ctx)
